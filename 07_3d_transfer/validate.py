...
def validate_age(config: dict, age: str) -> dict:
    ...
    for row, entry in zip(manifest.itertuples(index=False), entries):
        ...
        if workflow.sha256_file(path) != str(row.output_sha256):
            raise ValueError(f"{age}: output hash mismatch: {path}")
        ...
        workflow.validate_publication_lineage(
            result, config, age, entry, transport_contract, reference_artifacts
        )
        if workflow.matrix_sha256(result.X) != str(row.matrix_sha256):
            raise ValueError(f"{age}: matrix hash mismatch: {path}")
        declared = workflow.blueprint_contract_row(age, entry)
        for key in ("spot_ids_sha256", "xy_sha256", "xyz_sha256", "regions_sha256"):
            if str(getattr(row, key)) != str(declared[key]):
                raise ValueError(f"{age}: {key} mismatch at z-index {entry['z_index']}")
    ...
    if provenance.get("blueprint_sha256") != workflow.sha256_file(workflow.blueprint_path(config, age)):
        raise ValueError(f"{age}: consolidated blueprint hash mismatch")
    return {
        "age": age,
        "status": "validated",
        "n_z_levels": len(entries),
        "n_spots": sum(int(entry["n_spots"]) for entry in entries),
        "z_start": float(entries[0]["z_world"]),
        "z_end": float(entries[-1]["z_world"]),
...
def main() -> None:
    ...
    decision = {
        ...
        "status": "validated" if len(results) == 2 else "age_validated_only",
        "publication_canonical": False,
        "publication_status": (
            "candidate_identity_and_convergence_gate_passed_pending_article_metrics_visualization_and_author_review"
            if len(results) == 2
            else "single_age_gate_only"
        ),
        "feast": feast,
        "ages": results,
    }

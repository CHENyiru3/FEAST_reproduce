...
METRICS = (
    "ARI", "NMI", "AMI", "Homogeneity", "Completeness", "V_measure", "CHAOS", "PAS"
)
...
def grouped_summary(table: pd.DataFrame, source: str) -> list[dict[str, object]]:
    groups = {
        ...
        "all_simulations": table["simulation_id"].ne("real_baseline"),
        "altered_simulations": ~table["simulation_id"].isin(["baseline", "real_baseline"]),
        "feast_baseline": table["simulation_id"].eq("baseline"),
        "real_baseline": table["simulation_id"].eq("real_baseline"),
    }
    rows = []
        ...
        rows.append(
            {
                ...
                "group": group,
                "n_rows": len(values),
                **{metric: float(values[metric].mean()) for metric in METRICS},
            }
        )
    return rows
...
def main() -> int:
    ...
    for source, table in (
        ("prior_unsupervised", prior),
        ("label_informed", label_informed),
    ):
        for metric in METRICS:
            comparison[f"{source}_{metric}"] = table[metric]
            comparison[f"fresh_minus_{source}_{metric}"] = (
                fresh[metric] - table[metric]
            )
    ...
    summary_rows = []
    for source, table in (
        ("label_informed_historical", label_informed.reset_index()),
        ("prior_unsupervised", prior.reset_index()),
        ("fresh_unsupervised", fresh.reset_index()),
    ):
        ...
        baseline_rows.append(
            {
                "source": source,
                **{
                    f"{metric}_feast_minus_real": float(
                        source_rows.loc["feast_baseline", metric]
                        - source_rows.loc["real_baseline", metric]
                    )
    ...
    gaps_decreased = all(
        abs(fresh_gap[f"{metric}_feast_minus_real"])
        < abs(prior_gap[f"{metric}_feast_minus_real"])
        for metric in ("ARI", "NMI", "AMI")
    )
    decision = {
        "decision": "headline_direction_unchanged",
        "baseline_gap_absolute_magnitude_decreased_for_all_three_metrics": gaps_decreased,
        "publication_claim_authorized": False,
        "reason": (
            "Fresh label-free Leiden retains the FEAST-versus-real baseline-matching "
            "conclusion, but the combined GraphST/STAGATE benchmark is still pending."
        ),
    }
    ...
    provenance = {
        ...
        "inputs": {
            str(fresh_path.resolve()): sha256(fresh_path),
            str(prior_path.resolve()): sha256(prior_path),
            str(args.label_informed_table.resolve()): sha256(args.label_informed_table),
        },
        "outputs": {
            path.name: sha256(path)
            for path in (comparison_path, summary_path, baseline_path, decision_path)
        },
    }

...
METHOD_SCRIPTS = {
    "GraphST": "graphst.py",
    "STAGATE_mclust": "stagate_mclust.py",
    "Leiden_unsupervised": "leiden_unsupervised.py",
}
METHOD_CONFIG_KEYS = {
    "GraphST": "graphst",
    "STAGATE_mclust": "stagate",
    "Leiden_unsupervised": "leiden",
}
...
def method_metadata_is_valid(
    metadata_path: Path,
    method: str,
    settings: dict,
    public_seed: int,
    inherited_ld_library_path: str = "",
) -> tuple[bool, str]:
    ...
    if method == "GraphST":
        ...
        if not isinstance(gpu_memory, dict) or not _int_matches_positive(
            gpu_memory.get("peak")
        ):
        ...
        if environment.get("ld_library_path_sha256") != sha256_text(
            inherited_ld_library_path
        ):
            return False, "STAGATE LD_LIBRARY_PATH hash does not match"
    else:
        expected_resolutions = [float(value) for value in settings["resolutions"]]
        try:
            actual_resolutions = [
                float(value) for value in metadata.get("resolutions_requested", [])
            ]
        except (TypeError, ValueError):
        ...
        selected_index = max(
            range(len(sweep)),
            key=lambda index: (modularities[index], -sweep_resolutions[index]),
        )
...
def completed_output_is_valid(
    ...
    *,
    ...
    python_hash: str,
    inherited_ld_library_path: str,
    inherited_ld_library_path_hash: str,
) -> bool:
    required = {
    ...
    try:
        seed_matches = int(record.get("public_seed", -1)) == int(
            config["public_seed"]
        )
    except (TypeError, ValueError):
        seed_matches = False
    expected_fields = {
        ...
        "method_python": python_invocation,
        "method_python_resolved": python_resolved,
        "method_python_sha256": python_hash,
        "feast_version": "1.0.2",
        ...
        expected_fields.update(
            {
                ...
                "ld_library_path_sha256": inherited_ld_library_path_hash,
            }
        )
    if not (
        record
        ...
        and seed_matches
        and all(record.get(field) == expected for field, expected in expected_fields.items())
        and all(
            path.is_file() and sha256(path) == record.get(field)
            for field, path in required.items()
        )
    ):
        return False
    if method in {"GraphST", "STAGATE_mclust"} and not _is_true(
        record.get("cuda_execution_verified")
    ):
        return False
    metadata_valid, _ = method_metadata_is_valid(
...
def main() -> int:
    ...
    manifest = pd.read_csv(
        args.manifest, dtype={"slice_id": str, "simulation_id": str}
    )
    if (
        len(manifest) != 84
        or manifest[["slice_id", "simulation_id"]].duplicated().any()
    ...
    python_hash = sha256(python_resolved_path)
    inherited_ld_library_path = (
        os.environ.get("LD_LIBRARY_PATH", "")
        if args.method == "STAGATE_mclust"
        else ""
    )
    inherited_ld_library_path_hash = sha256_text(inherited_ld_library_path)
    ...
    for index, row in enumerate(manifest.itertuples(index=False), start=1):
        ...
        if source_hash != row.output_sha256:
            raise RuntimeError(f"fixed-panel checksum mismatch: {source}")
        output = method_root / row.slice_id / row.simulation_id
        old = previous.get((row.slice_id, row.simulation_id), {})
        if args.resume and completed_output_is_valid(
            output,
            old,
            source_hash,
            config=config,
            ...
            inherited_ld_library_path_hash=inherited_ld_library_path_hash,
        ):
            rows.append(old)
            print(
                f"[{index}/84] {args.method} {row.slice_id}/{row.simulation_id}: "
                "skipped verified",
                flush=True,
            )
            continue
        ...
        if args.method == "GraphST":
            ...
            command += [
                "--resolutions",
                ",".join(map(str, settings["resolutions"])),
                "--n-pcs",
                str(settings["n_pcs"]),
                "--n-neighbors",
                str(settings["n_neighbors"]),
            ]
        ...
        files_complete = all(path.is_file() for path in artifacts.values())
        ...
        rows.append(
            {
                ...
                **{
                    field: sha256(path) if path.is_file() else ""
                    for field, path in artifacts.items()
                },
                "runner_log_sha256": sha256(runner_log),
                "public_seed": config["public_seed"],
                "feast_version": "1.0.2",
                "feast_commit": config["required_feast_commit"],
                "method_status": metadata.get("status", ""),
                "cuda_execution_verified": metadata.get(
    ...
    provenance = {
        ...
        "job_count": len(rows),
        "successful_jobs": sum(row["status"] == "ok" for row in rows),
    ...
    (method_root / "provenance.json").write_text(

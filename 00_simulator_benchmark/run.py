...
def main() -> int:
    ...
    expected_hashes = inputs[inputs.role == "reference"].set_index("sample").sha256
    input_hashes = {sample: sha256(path) for sample, path in jobs}
    mismatched = [
        sample
        for sample, digest in input_hashes.items()
        if digest != expected_hashes.loc[sample]
    ]
    if mismatched:
        raise RuntimeError(f"reference checksum mismatch: {mismatched}")
    ...
    for index, (sample, source) in enumerate(jobs, start=1):
        started = time.time()
        input_hash = input_hashes[sample]
        ...
        if (
            ...
            and sha256(output_path) == old.get("output_sha256")
        ):
            rows.append(old)
        ...
        if output_path.exists():
            failure = (
                args.output_dir
                / "failures"
                / f"{sample}__{int(time.time())}.h5ad"
            )
        ...
        try:
            ...
            annotation_key = next(
                (name for name in LABEL_COLUMNS if name in reference.obs), None
            )
            simulated = FEAST.simulate(
                ...
                beta_early_stopping_patience=int(
                    feast_config["beta_early_stopping_patience"]
                ),
            ...
            simulated.uns["publication_provenance"] = {
                ...
                "solver_diagnostics": {
                    "status": "completed",
                    "assignment_method": assignment_diagnostics.get(
                        "assignment_method"
                    ),
                    "assignment_blocks": False,
                ...
                "input_artifact_id": f"study00:reference:{sample}",
            ...
            row = {
                ...
                "runner_source_sha256": runner_hash,
                "sample": sample,
                "simulator": feast_config["simulator_name"],
                "file_path": output.as_posix(),
                "input_sha256": input_hash,
                "output_sha256": sha256(output_path),
                "n_obs": simulated.n_obs,
                "n_vars": simulated.n_vars,

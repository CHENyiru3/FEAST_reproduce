...
SIMULATION_DIR = REPOSITORY_ROOT / '01_clustering' / 'outputs' / 'final_rerun_20260718' / 'simulations'
SIMULATION_MANIFEST = SIMULATION_DIR / 'simulation_manifest.csv'
SLICES = ('151508', '151670', '151676')
FACTOR_SPECS = {'mean': {'title': 'Mean control', 'x_label': 'Nominal fold change', 'neutral': 1.0, 'levels': ((0.5, 'mean_0_50'), (0.67, 'mean_0_67'), (0.8, 'mean_0_80'), (1.0, 'baseline'), (1.25, 'mean_1_25'), (1.5, 'mean_1_50'), (2.0, 'mean_2'))}, 'variance': {'title': 'Variance control', 'x_label': 'Nominal fold change', 'neutral': 1.0, 'levels': ((0.5, 'variance_0_50'), (0.67, 'variance_0_67'), (0.8, 'variance_0_80'), (1.0, 'baseline'), (1.25, 'variance_1_25'), (1.5, 'variance_1_50'), (2.0, 'variance_2'))}, 'sparsity': {'title': 'Zero-fraction control', 'x_label': 'Nominal logit shift', 'neutral': 0.0, 'levels': ((-1.0, 'sparsity_neg_1'), (-0.5, 'sparsity_neg_0_50'), (-0.25, 'sparsity_neg_0_25'), (0.0, 'baseline'), (0.25, 'sparsity_pos_0_25'), (0.5, 'sparsity_pos_0_50'), (1.0, 'sparsity_pos_1'))}}
METRIC_SPECS = (('mean', 'Realized mean\n(relative to baseline)'), ('variance', 'Realized variance\n(relative to baseline)'), ('zero_prop', 'Realized zero fraction\n(relative to baseline)'))
...
def _manifest_index() -> pd.DataFrame:
    manifest = pd.read_csv(SIMULATION_MANIFEST, dtype={'slice_id': str, 'simulation_id': str})
            ...
            for slice_id in SLICES:
                ...
                if key not in cached_diagnostics:
                    ...
                    try:
                        ...
                        if missing_metrics:
                            raise ValueError(f'{path} lacks realized diagnostics: {sorted(missing_metrics)}')
                        cached_diagnostics[key] = {metric: float(realized[metric]) for metric in required_metrics}
                    finally:
                        data.file.close()
                    input_paths.append(path)
                rows.append({'slice_id': slice_id, 'factor': factor, 'simulation_id': simulation_id, 'nominal_level': nominal_level, **cached_diagnostics[key]})
...
def draw_figure(data: pd.DataFrame, output_dir: Path, dpi: int) -> list[Path]:
    ...
    for row_idx, (metric, metric_label) in enumerate(METRIC_SPECS):
        ...
        for col_idx, factor in enumerate(factors):
            ...
            for slice_id in SLICES:
                ...
                ax.plot(slice_data['nominal_level'], slice_data[metric], color=SLICE_COLOR, linewidth=0.9, alpha=0.35, zorder=1)
            summary = subset.groupby('nominal_level', as_index=False)[metric].mean()
            ax.plot(summary['nominal_level'], summary[metric], color=SUMMARY_COLOR, marker='o', markerfacecolor='white', markeredgewidth=1.1, markersize=5.5, linewidth=2.3, zorder=3)
            ax.axhline(1.0, color='#4D4D4D', linewidth=0.9, linestyle='--', alpha=0.8, zorder=0)
            ax.axvline(FACTOR_SPECS[factor]['neutral'], color='#CFCECE', linewidth=0.8, linestyle=':', zorder=0)
    ...
    handles = [Line2D([0], [0], color=SLICE_COLOR, linewidth=1.2, alpha=0.6, label='Individual slice'), Line2D([0], [0], color=SUMMARY_COLOR, marker='o', markerfacecolor='white', markeredgewidth=1.1, linewidth=2.3, label='Three-slice mean')]
    fig.legend(handles=handles, loc='upper right', bbox_to_anchor=(0.98, 0.987), ncol=2, fontsize=10, handlelength=1.7, columnspacing=1.4)
    fig.text(0.08, 0.985, 'Realized FEAST simulation intervention profile', ha='left', va='center', fontsize=14, fontweight='bold')
    fig.text(0.08, 0.948, 'Recorded simulator diagnostics; horizontal dashed line = baseline 1.0, vertical dotted line = neutral setting.', ha='left', va='center', fontsize=9.2, color='#666666')
    fig.subplots_adjust(left=0.095, right=0.995, top=0.9, bottom=0.095, hspace=0.34, wspace=0.25)
    output_dir.mkdir(parents=True, exist_ok=True)
    stem = output_dir / 'intervention_profile'
    paths: list[Path] = []
    for suffix, kwargs in {'pdf': {'metadata': {'Creator': 'FEAST Study 01 intervention profile'}}, 'png': {'dpi': dpi, 'metadata': {'Software': 'FEAST Study 01 intervention profile'}}, 'svg': {'metadata': {'Creator': 'FEAST Study 01 intervention profile'}}}.items():
...
def write_provenance(output_dir: Path, outputs: list[Path], input_paths: list[Path]) -> Path:
    payload = {'schema_version': 1, 'figure_set': 'study01_intervention_profile', 'source': {'plot_script': str(Path(__file__).resolve().relative_to(REPOSITORY_ROOT)), 'simulation_manifest': str(SIMULATION_MANIFEST.relative_to(REPOSITORY_ROOT)), 'diagnostic_field': 'uns.alteration_diagnostics.realized_stage_achieved_change', 'n_simulation_files_read': len(input_paths)}, 'design': {'slices': list(SLICES), 'factors': list(FACTOR_SPECS), 'metrics': [metric for metric, _label in METRIC_SPECS], 'baseline_reference': 1.0, 'summary_encoding': 'thin individual-slice traces; blue three-slice mean'}, 'outputs': {path.name: None for path in sorted(outputs)}, 'scientific_disposition': {'figure_promotion_authorized': False, 'supported_interpretation': 'Descriptive profile of FEAST-recorded realized simulation diagnostics across the registered three-slice cohort.'}}

...
STUDIES = ('00_simulator_benchmark', '01_clustering', '02_alignment', '03_deconvolution', '04_batch_effect_removal', '05_2d_conditional_transfer', '06_3d_stack', '07_3d_transfer')
...
def required_commit_from_config(path: Path) -> str:
    match = re.search('(?m)^required_feast_commit:\\s*[\\"\']?([0-9a-f]{40})[\\"\']?\\s*$', path.read_text(encoding='utf-8'))
    if match is None:
...
def main() -> int:
    ...
    if manifest['release_authorized']:
        incomplete = [study for study, record in studies.items() if record.get('status') != 'validated_complete']
        if manifest['status'] != 'approved' or incomplete or (not manifest.get('final_tag')):
            raise ValueError('authorized release requires approved status, a final tag, and complete studies')
    ...
    expected = {'version': record['Package version'].split()[0], 'source_commit': record['Required source commit'], 'python': record['Required Python'], 'numpy': record['Required NumPy']}
    ...
    print(f"publication manifest: OK (status={manifest['status']}, authorized={manifest['release_authorized']})")
    return 0

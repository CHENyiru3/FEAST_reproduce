...
from score import integer_libraries, metrics, read_composition, sha256_file, with_other
...
def score_resolution(
    simulation_row: pd.Series, rctd_row: pd.Series, candidate_dir: Path,
    ...
    for path, checksum, label in (
        (simulation_path, str(simulation_row["simulation_sha256"]), "simulation"),
        (truth_path, str(simulation_row["truth_sha256"]), "truth"),
        (reference_path, str(simulation_row["reference_sha256"]), "reference"),
        (Path(rctd_row["output_path"]), str(rctd_row["output_sha256"]), "RCTD output"),
    ):
        require_hash(path, checksum, f"{pair_id} {label}")
    ...
    if (
        candidate_metadata.get("input_sha256") != str(simulation_row["simulation_sha256"])
        or candidate_metadata.get("reference_sha256") != str(simulation_row["reference_sha256"])
        or candidate_metadata.get("output_sha256") != sha256_file(candidate_path)
    ):
        raise RuntimeError(f"candidate Cell2location provenance is not closed: {pair_id}")
    ...
    if not np.allclose(rctd.sum(axis=1), 1.0, atol=1e-5):
        raise RuntimeError(f"RCTD rows are not normalized: {pair_id}")
    if not np.allclose(candidate.loc[positive_spots].sum(axis=1), 1.0, atol=1e-6):
    ...
    for method, prediction in (("rctd", rctd), ("cell2location", candidate.loc[positive_spots])):
        rows.append({
            ...
            "status": "ok",
            "comparison_scope": "same_expression_exact_positive_spots_exact_61_named_types_plus_other",
            ...
            **metrics(truth_scored, with_other(prediction, common)),
        })
    audit = {
        "pair_id": pair_id,
        "slice_id": slice_id,
        ...
        "n_positive_spots": len(positive_spots),
        "n_zero_library_spots": len(zero_spots),
        "n_truth_fine_types": len(truth.columns),
        "n_exact_retained_named_types": len(common),
        "rctd_and_candidate_column_sets_equal": True,
        "candidate_positive_spot_order_exact": True,
        "candidate_zero_rows_zero": True,
        "canonical_common_cell_type_order_json": json.dumps(list(common)),
    }
    lineage = [
        {"pair_id": pair_id, "artifact": "simulation", **artifact(simulation_path)},
        {"pair_id": pair_id, "artifact": "truth", **artifact(truth_path)},
        {"pair_id": pair_id, "artifact": "rctd", **artifact(Path(rctd_row["output_path"]))},
        {"pair_id": pair_id, "artifact": "cell2location_corrected", **artifact(candidate_path)},
        {"pair_id": pair_id, "artifact": "cell2location_corrected_metadata", **artifact(candidate_metadata_path)},
    ]
...
def main() -> int:
    ...
    parser.add_argument("--resolution", type=float, action="append", default=[0.1, 0.25])
    args = parser.parse_args()
    if args.output_dir.exists():
        raise FileExistsError(f"refusing to overwrite {args.output_dir}")
    resolutions = tuple(dict.fromkeys(float(value) for value in args.resolution))
    if not resolutions:
    ...
    payload = {
        "schema_version": 1,
        "analysis": "slice100_cell2location_label_matched_corrective_diagnostic",
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "publication_status": "diagnostic_only_not_an_aggregate_method_ranking",
        "scope": {
            "slice_id": str(args.slice_id),
            "resolutions": list(resolutions),
            "methods": ["rctd", "cell2location"],
            "comparison": "same expression, exact positive spots, exact retained 61 named fine labels plus Other",
        },
        "source": {
        ...
        "outputs": {path.name: sha256_file(path) for path in (scores_path, audit_path, lineage_path)},
    }

...
METHOD_SPECS = (
    ("GraphST", "GraphST", "#0072B2", "o"),
    ("STAGATE_mclust", "STAGATE + mclust", "#E69F00", "s"),
    ("Leiden_unsupervised", "Label-free Leiden", "#009E73", "^"),
)
FACTOR_SPECS = {
    "mean": {
        "title": "Mean control",
        "x_label": "Nominal fold change (neutral = 1)",
        "neutral": 1.0,
        "levels": (
    ...
    "sparsity": {
        ...
        "x_label": "Nominal logit shift (neutral = 0)",
        "neutral": 0.0,
        "levels": (
...
def configure_matplotlib() -> None:
    ...
    plt.rcParams.update({
        ...
        "ps.fonttype": 42,
        "svg.fonttype": "none",
        "svg.hashsalt": "feast-study01-clustering-stability-v1",
    })


def registered_metric_source() -> tuple[Path, str]:
    ...
    return REPOSITORY_ROOT / candidate["metrics"], str(candidate["metrics_sha256"])
...
def verify_registered_file(path: Path, expected_hash: str) -> None:
    ...
    if observed_hash != expected_hash:
        raise ValueError(
            f"Registered SHA-256 mismatch for {path}: expected {expected_hash}, "
...
def _load_run_manifests() -> dict[str, pd.DataFrame]:
    ...
    for method, _label, _color, _marker in METHOD_SPECS:
        ...
        required = {"slice_id", "simulation_id", "clusters_sha256", "status"}
...
def _read_labels(path: Path, expected_hash: str) -> pd.Series:
    if not path.is_file():
        raise FileNotFoundError(f"Cluster file is missing: {path}")
    observed_hash = sha256(path)
    if observed_hash != expected_hash:
        raise ValueError(f"Cluster checksum mismatch for {path}")
...
def load_stability_data() -> tuple[pd.DataFrame, dict[str, str], int]:
    metric_path, metric_hash = registered_metric_source()
    verify_registered_file(metric_path, metric_hash)
    ...
    def get_labels(method: str, slice_id: str, simulation_id: str) -> pd.Series:
        ...
        if key not in labels_cache:
            ...
            expected_hash = str(manifest.loc[manifest_key, "clusters_sha256"])
            labels_cache[key] = _read_labels(path, expected_hash)
            verified_paths[str(path.relative_to(REPOSITORY_ROOT))] = expected_hash
                ...
                for nominal_level, simulation_id in spec["levels"]:
                    ...
                    if not baseline.index.equals(perturbed.index):
                        if set(baseline.index) != set(perturbed.index):
                            raise ValueError(
                                f"Spot set changed for {method}/{slice_id}/{simulation_id}"
                            )
                    ...
                    rows.append({
                        ...
                        "partition_change_ari": 1.0 - stability_ari,
                    })

    data = pd.DataFrame(rows)
    expected_rows = len(SLICES) * len(METHOD_SPECS) * sum(
        len(spec["levels"]) for spec in FACTOR_SPECS.values()
    )
...
def draw_figure(data: pd.DataFrame, output_dir: Path, dpi: int) -> list[Path]:
    ...
    fig, axes = plt.subplots(1, len(factors), figsize=(13.0, 5.1))
    ...
    for col_idx, factor in enumerate(factors):
        ...
        for method, _method_label, color, marker in METHOD_SPECS:
            ...
            for slice_id in SLICES:
                slice_data = method_data.loc[
                    method_data["slice_id"] == slice_id
                ].sort_values("nominal_level")
                ax.plot(
                    slice_data["nominal_level"],
                    slice_data[PLOT_METRIC],
                    color=color,
                    linewidth=0.85,
                    alpha=0.28,
                    zorder=1,
        ...
        if col_idx == 0:
            ax.set_ylabel("Partition change from\nFEAST baseline (1 − ARI) ↑",
                          fontsize=10.5, fontweight="bold", labelpad=6)
    ...
    handles = [
        Line2D([0], [0], color=color, marker=marker, markerfacecolor="white",
               markeredgewidth=0.9, linewidth=2.25, markersize=6,
               label=label)
        for _method, label, color, marker in METHOD_SPECS
    ]
    # Reserve three separate header rows for title, interpretation, and legend.
    # Keeping the legend out of the subtitle's reading line avoids the ambiguity
    # caused by the former compact multi-metric layout.
    fig.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.55, 0.910),
               ncol=3, fontsize=10, handlelength=1.8, columnspacing=1.4)
    fig.text(0.08, 0.976, "How much does the predicted partition change?",
             ha="left", va="center", fontsize=14, fontweight="bold")
    fig.text(
        0.08,
        0.933,
        "1 − ARI versus the same-method FEAST baseline: 0 = identical partition; higher = stronger reorganization. Thin = slice; solid = mean.",
        ha="left",
        va="center",
        fontsize=8.8,
        color="#666666",
    )
    fig.subplots_adjust(left=0.095, right=0.995, top=0.785, bottom=0.15, wspace=0.25)

    output_dir.mkdir(parents=True, exist_ok=True)
    stem = output_dir / "clustering_stability_vs_baseline"
    paths: list[Path] = []
    for suffix, kwargs in {
        "pdf": {"metadata": {"Creator": "FEAST Study 01 clustering stability"}},
        "png": {"dpi": dpi, "metadata": {"Software": "FEAST Study 01 clustering stability"}},
        "svg": {"metadata": {"Creator": "FEAST Study 01 clustering stability"}},
    }.items():
...
def write_provenance(
    ...
    payload = {
        "schema_version": 1,
        "figure_set": "study01_partition_change_from_baseline",
        "source": {
            ...
            "registered_metric_source_sha256": metric_hash,
            "clusters_csv_verified": verified_cluster_count,
            "cluster_hashes": verified_paths,
        },
        "design": {
            "slices": list(SLICES),
            "factors": list(FACTOR_SPECS),
            "display_metric": "1 - stability_ari",
            "data_metrics": ["stability_ari", "stability_nmi"],
            "reference_partition": "same-method FEAST baseline",
            "label_remapping": "none; ARI and NMI are label-permutation-invariant",
        },
        "outputs": {path.name: sha256(path) for path in sorted(outputs)},
        "scientific_disposition": {
            "figure_promotion_authorized": False,
            "supported_interpretation": (
                "Descriptive within-method partition change across controlled simulation "
...
def main() -> int:
    ...
    print(
        "Saved: "
        f"{write_provenance(args.output_dir, outputs, verified_paths, verified_cluster_count)}"
    )
    return 0

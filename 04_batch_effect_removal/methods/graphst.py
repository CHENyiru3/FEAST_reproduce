...
def run_graphst_batch_correction(
    ...
    directed, graph_meta = build_within_batch_knn(
        adata_hvg.obsm["spatial"],
        adata_hvg.obs[batch_key].astype(str).to_numpy(),
        n_neighbors=3,
    )
    ...
    finite_parameters = bool(parameter_tensors) and all(
    ...
    loss_components = {
        "feature": float(model.loss_feat.detach().cpu()),
        "contrastive_1": float(model.loss_sl_1.detach().cpu()),
        "contrastive_2": float(model.loss_sl_2.detach().cpu()),
    }
    loss_components["weighted_total"] = float(
        model.alpha * loss_components["feature"]
        + model.beta
        * (loss_components["contrastive_1"] + loss_components["contrastive_2"])
    )
    if (
        not finite_parameters
        or not finite_optimizer_state
        or not np.isfinite(list(loss_components.values())).all()
    ):
    ...
    meta = {
        ...
        "embedding_source_key": embedding_source_key,
        "embedding_dimensions": int(adata_hvg.obsm["X_GraphST"].shape[1]),
        "seed": seed,
        ...
        "training": {
            ...
            "all_optimizer_state_tensors_finite": finite_optimizer_state,
            "model_parameter_tensors": int(len(parameter_tensors)),
            "model_parameter_values": int(
                sum(parameter.numel() for parameter in parameter_tensors)
            ),
            "optimizer_state_tensors": int(len(optimizer_tensors)),
            "optimizer_state_values": int(
                sum(value.numel() for value in optimizer_tensors)
            ),
        ...
        "graphst_import": {
            "path": str(Path(graphst_module.__file__).resolve()),
            "sha256": sha256_file(Path(graphst_module.__file__).resolve()),
        },
        "environment": {
            ...
            "graphst": _distribution_version("GraphST"),
            ...
            "external_environment_limitation": (
                ...
                else "This external GraphST worker uses a NumPy version unsupported "
...
def main():
    ...
    try:
        adata_out, meta, model = run_graphst_batch_correction(
            adata_ref,
            adata_query,
        ...
        torch.save(
            {
                ...
                "seed": args.seed,
                "panel_ordered_gene_sha256": panel_metadata[
                    "ordered_gene_sha256"
                ],
            },
            checkpoint_path,
        ...
        meta["provenance"] = build_provenance(
            ...
            outputs=[embeddings_path, result_path, checkpoint_path],
            solver_diagnostics={
                "status": "completed",
                "epochs_requested": args.epochs,
                ...
                "all_optimizer_state_tensors_finite": meta["training"][
                    "all_optimizer_state_tensors_finite"
                ],
                "final_loss_components": meta["training"][

...
def main() -> int:
    ...
    if runtime["wheel_sha256"] != str(config["required_wheel_sha256"]):
        raise RuntimeError("clean FEAST wheel hash differs from config.yaml")
    ...
    blueprint_obs = pd.DataFrame(
        {
            spot_id_column: target.obs_names.astype(str).to_numpy(),
            annotation_key: target_labels,
        }
    )
    blueprint = SimulationBlueprint(
        coordinates=np.asarray(target.obsm["spatial"], dtype=np.float64),
        domain_map=target_labels,
        obs=blueprint_obs,
        metadata={
            "artifact_id": input_artifact_id(config, job.dataset, job.target),
            "expression_policy": config["target_expression_policy"],
            "mode": job.mode,
            "dataset": job.dataset,
    ...
    work_dir = Path(
        tempfile.mkdtemp(prefix=f"{job_id(config, job)}-", dir=work_root)
    )
    ...
    # FEAST v1 resets blueprint row indices. Verify the immutable ID column,
    # labels, coordinates, and genes before rebinding the public obs_names.
    validate_generated(
        generated,
        target,
        genes,
    ...
    embedded_support_hash = sha256_json(support)
    generated.uns["feast_reproduce"] = {
        "configuration_id": configuration_id,
        "study_configuration_id": str(config["configuration_id"]),
        "mode": job.mode,
        "dataset": job.dataset,
        ...
        "source_artifact_id": input_artifact_id(config, job.dataset, job.source),
        "source_sha256": sha256_file(source_path),
        "target_artifact_id": input_artifact_id(config, job.dataset, job.target),
        "target_sha256": sha256_file(target_path),
        "ordered_gene_panel_sha256": sha256_lines(genes),
        "support_sha256": embedded_support_hash,
        ...
        "target_expression_policy": str(config["target_expression_policy"]),
        "target_expression_loaded_by_generation": False,
        "identity_verified_before_obs_name_rebinding": True,
    ...
    provenance = {
        ...
        "input_artifacts": {
            "source": {
                "artifact_id": input_artifact_id(config, job.dataset, job.source),
        ...
        "numerical_change": {
            "field": "sinkhorn_iter",
            "historical": 200,
            "fresh": int(transport["sinkhorn_iter"]),

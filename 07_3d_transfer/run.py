...
def assignment_randomness(config: dict, age: str) -> tuple[float, dict]:
    ...
    if payload['configuration_id'] != config['configuration_id'] or not payload['reference_only']:
        raise ValueError('calibration must match this configuration and use reference holdouts')
    for key in ('n_references', 'min_positions', 'batch_sd'):
        if payload[key] != config['generation'][key]:
            raise ValueError(f'calibration generation setting changed: {key}')
    if payload['simulation_config'] != asdict(simulation_config(config, 0.0)):
        raise ValueError('calibration spatial/count settings differ from generation')
    if payload['feast']['commit'] != config['required_feast_commit']:
        raise ValueError('calibration package lineage differs')
    return float(payload['observed_assignment_randomness']), workflow.assignment_randomness_provenance(config, age)

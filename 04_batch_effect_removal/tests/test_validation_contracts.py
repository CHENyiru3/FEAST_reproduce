...
import compare_historical as historical  # noqa: E402
...
def test_historical_input_records_use_explicit_resolved_paths(tmp_path: Path) -> None:
    ...
    assert historical.pinned_input_records(atomic, composite) == [
        {
            "role": "historical_comparable_atomic_metrics",
            "path": str(atomic.resolve()),
...
def test_validator_owned_metric_formulas_match_scorer_on_synthetic_data() -> None:
    ...
    independently_recomputed = validate.validator_metric_row(
        method="GraphST",
        representation="synthetic",
        primary=True,
        embedding=embedding,
    ...
    pd.testing.assert_series_equal(
        pd.Series(observed),
        pd.Series(independently_recomputed),
        check_exact=False,
        rtol=1e-13,
        atol=1e-13,
    )
...
def test_validator_owned_pca_and_expression_paths_match_scorer() -> None:
    ...
    validator_representations = validate.validator_representations(
        "GraphST", embedding, 42
    )
    for scorer_value, validator_value in zip(
        scorer_representations, validator_representations, strict=True
    ):
    ...
    assert score.paired_expression_correlation(expression, 4, names) == pytest.approx(
        validate.validator_paired_expression_correlation(expression, 4, names),
        rel=1e-13,
        abs=1e-13,
    )
...
def test_live_retry_binding_rejects_a_second_parameter_change() -> None:
    candidate = (
        STUDY_ROOT
        / "outputs/final_rerun_20260718_v2/methods/STAMP/shift_only/alpha_1.50"
    )

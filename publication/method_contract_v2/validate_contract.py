...
METHODS = {
    "generative_parameter_assignment",
    "reference_rank",
    "conditional_empirical_rank",
    "conditional_imputation",
    "batch_parameter_deformation",
}
STATUSES = {
    "validated_candidate",
    "in_progress",
...
EXPECTED_PREDECESSORS = {
    "00_simulator_benchmark": "FEAST-ART-00-SIM-v1",
    "01_clustering": "FEAST-ART-01-CLUSTER-v1",
    "02_alignment": "FEAST-ART-02-ALIGN-v1",
    "03_deconvolution": "FEAST-ART-03-DECONV-v1",
    "04_2d_conditional_transfer": "FEAST-ART-04-2D-XSLICE-v1",
    "05_3d_stack": "FEAST-ART-05-3D-IMPUTE-v1",
    "06_3d_transfer": "FEAST-ART-06-DEVCCF-E15-v1",
    "08_batch_effect_removal": "FEAST-ART-08-BATCH-v1",
}
CONDITIONAL_STUDIES = {
    "04_2d_conditional_transfer",
    "05_3d_stack",
    "06_3d_transfer",
}
...
def validate_file(path: Path) -> tuple[dict, list[str]]:
    ...
    if study in CONDITIONAL_STUDIES:
        ...
        expected_counts = {
            "active_outputs": 156,
            "explicit_failure_saved": 31,
            "failure_tainted": 26,
            "missing_positive_convergence_evidence": 99,
            "verifiably_converged": 0,
            "canonical_outputs": 0,
        }
    ...
    for collection_name, entries in references:
        for entry in entries:
            ...
            if not isinstance(alias_path, str) or not isinstance(expected_hash, str):
                errors.append(f"{collection_name} entries require path and sha256 strings")
                continue
            try:
            ...
            elif sha256(evidence_path) != expected_hash:
                errors.append(f"bound-file SHA-256 mismatch: {alias_path}")
...
def main() -> int:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print(f"Validated {len(paths)} v2 candidate workflow contracts and evidence hashes.")
    return 0

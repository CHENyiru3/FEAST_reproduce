...
tryCatch({
    ...
    cat("Splat: Estimating parameters...\n")
    params <- splatEstimate(as.matrix(counts_in))

    cat("Splat: Simulating counts...\n")
    sim <- tryCatch(
        splatSimulate(
            params,
        ...
        error = function(e) {
            cat("Splat: estimate-based simulation failed, using default params.\n")
            splatSimulate(
                nGenes = nrow(counts_in),
                batchCells = ncol(counts_in),
                verbose = FALSE
    ...
    cat("Splat: Extracting simulated data and integrating spatial locations...\n")
    ...
    cat("Splat: Saving intermediate files to:", opt$output_dir, "\n")
    writeMM(sim_counts, file.path(opt$output_dir, "splat_count.mtx"))
    write.csv(data.frame(gene = rownames(sim_counts)),
              file.path(opt$output_dir, "splat_genes.csv"),
              row.names = FALSE)
    write.csv(loc, file.path(opt$output_dir, "splat_loc.csv"), row.names = FALSE)
}, error = function(e) {
    cat("Splat error:\n")

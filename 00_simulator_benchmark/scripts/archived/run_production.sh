...
# 6 simulators; FEAST has 2 decoder modes
FEAST_MODES="raw reference_rank"
declare -A FEAST_LABEL=(["raw"]="FEAST_OT" ["reference_rank"]="FEAST_Rank")
declare -A FEAST_DECODE_METHOD=(["raw"]="quantile" ["reference_rank"]="rank")
declare -A FEAST_ASSIGNMENT_METHOD=(["raw"]="hybrid_ot" ["reference_rank"]="copula_rank_ot")
...
print_status() {
    ...
    for folder in FEAST_OT FEAST_Rank srtsim_updated splat splatSimple sccube; do
...
stage_3() {
    ...
    for mode in ${FEAST_MODES}; do
        local label="${FEAST_LABEL[${mode}]}"
        local decode_method="${FEAST_DECODE_METHOD[${mode}]}"
        local assignment_method="${FEAST_ASSIGNMENT_METHOD[${mode}]}"
        local out_dir="${OUTPUTS}/${label}"
        mkdir -p "${out_dir}"
        ...
        for sample in "${SAMPLES[@]}"; do
            ...
            local sample_log="${LOG_DIR}/feast_${label}_${sample}.log"
            log "  [RUN]  ${label}:${sample} [$(($done_count + 1))/${total}]"

            if "${PYTHON}" scripts/ParameterCloud_sim.py \
                --input "${input_path}" \
                --output "${output_path}" \
                --decode-method "${decode_method}" \
                --quantile-calibration "${mode}" \
                ...
                fail_record "03_feast" "${label}" "${sample}"
                done_count=$((done_count + 1))
            fi
        done
    done
    ...
    if [[ ${remaining} -eq 0 ]]; then
        log "  Stage 3 complete — ${total} outputs"

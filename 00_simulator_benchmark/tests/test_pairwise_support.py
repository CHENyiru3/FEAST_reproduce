...
def test_exact_identifiers_are_preferred() -> None:
    ...
    reference_indices, simulated_indices, diagnostics = SCORE.paired_spot_indices(reference, simulated)
...
def test_complete_coordinate_bijection_recovers_permutation() -> None:
    ...
    assert diagnostics['coordinate_max_abs_delta'] == 0.0

def test_coordinate_rounding_tolerates_serialization_noise() -> None:
    reference = make_data(['a', 'b'], np.array([[0.123456789, 1], [2, 3]]))
    simulated = make_data(['C_1', 'C_2'], np.array([[2 + 1e-12, 3], [0.123456789, 1]]))
    reference_indices, simulated_indices, diagnostics = SCORE.paired_spot_indices(reference, simulated, coordinate_decimals=8)
    assert reference_indices.tolist() == [0, 1]
    assert simulated_indices.tolist() == [1, 0]
    assert diagnostics['coordinate_max_abs_delta'] <= 1e-08
...
def test_pair_metrics_respects_coordinate_permutation() -> None:
    ...
    metrics, _ = SCORE.pair_metrics(reference, simulated, max_genes=12, neighbors=1, coordinate_decimals=8)

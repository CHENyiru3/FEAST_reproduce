...
def _serialize_history(history) -> tuple[dict, dict]:
    ...
    for key, value in history.items():
        ...
        for row in values:
            ...
            for item in row:
                ...
                serialized_row.append(
                    numeric_item if np.isfinite(numeric_item) else None
                )
        ...
        summary[key] = {
            ...
            "last": float(finite[-1]) if finite.size else None,
...
def run_scvi_batch_correction(
    ...
    # Decode every spot under the same reference-batch condition. Using the
    # observed batch here would normalize library size but would not remove the
    # learned batch covariate from the expression output.
    ...
    all_history_values_finite = bool(history_summary) and all(
        item["finite_values"] == item["total_values"]
        for item in history_summary.values()
    )
    ...
    finite_model_parameters = bool(model_parameters) and all(
    ...
    state_tensors = [
        value for value in model.module.state_dict().values() if torch.is_tensor(value)
    ]
    finite_model_state = bool(state_tensors) and all(
        bool(torch.isfinite(value).all().item()) for value in state_tensors
    )
    ...
    for callback in getattr(model.trainer, "callbacks", []):
        ...
        early_stopping = {
            ...
            "wait_count": getattr(callback, "wait_count", None),
    ...
    meta = {
        ...
        "normalized_expression_transform_batch": "ref",
        "seed_application": {
            "python_random": seed,
            "numpy": seed,
            "torch": seed,
            "torch_cuda_all": seed if torch.cuda.is_available() else "not_available",
            "scvi_settings": int(scvi.settings.seed),
            "lightning_deterministic": True,
        },
        "training": {
            "epochs_recorded": int(max(history_lengths)),
            "trainer_current_epoch": int(model.trainer.current_epoch),
            "trainer_should_stop": bool(model.trainer.should_stop),
            "early_stopping": early_stopping,
            "history_summary": history_summary,
            "all_history_values_finite": all_history_values_finite,
            "latent_all_finite": True,
            "normalized_expression_all_finite": True,
            "normalized_expression_nonnegative": True,
            "all_model_parameters_finite": finite_model_parameters,
            "all_model_state_tensors_finite": finite_model_state,
            "model_parameter_tensors": int(len(model_parameters)),
            "model_parameter_values": int(
                sum(parameter.numel() for parameter in model_parameters)
            ),
            "model_state_tensors": int(len(state_tensors)),
            "model_state_values": int(sum(value.numel() for value in state_tensors)),
        },
        ...
        "execution_environment": {
            ...
            "external_environment_limitation": (
                ...
                else "This external scVI worker uses a NumPy version unsupported "
...
def main():
    ...
    try:
        adata_out, meta, model = run_scvi_batch_correction(
            adata_ref,
            adata_query,
        ...
        meta["provenance"] = build_provenance(
            ...
            outputs=[embeddings_path, result_path, *model_files],
            solver_diagnostics={
                "status": "completed",
                "embedding_all_finite": bool(
                    np.isfinite(adata_out.obsm["X_scVI"]).all()
                ),
                "early_stopping": True,
                "max_epochs": args.max_epochs,
                "learning_rate": args.learning_rate,
                "epochs_recorded": meta["training"]["epochs_recorded"],
                "trainer_should_stop": meta["training"]["trainer_should_stop"],
                "latent_all_finite": meta["training"]["latent_all_finite"],
                "normalized_expression_all_finite": meta["training"][
                    "normalized_expression_all_finite"
                ],
                "all_model_parameters_finite": meta["training"][
                    "all_model_parameters_finite"
                ],
                "all_model_state_tensors_finite": meta["training"][
                    "all_model_state_tensors_finite"
                ],
                "all_history_values_finite": meta["training"][

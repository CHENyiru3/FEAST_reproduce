...
def neighbor_indices(coords: np.ndarray, neighbors_including_self: int) -> list[np.ndarray]:
    ...
    raw = NearestNeighbors(n_neighbors=n_neighbors).fit(coordinates).kneighbors(
...
def _pearson_columns(first: np.ndarray, second: np.ndarray) -> np.ndarray:
    ...
    denominator = np.sqrt(
        np.sum(first_centered * first_centered, axis=0)
        * np.sum(second_centered * second_centered, axis=0)
    )
...
def _moran_columns(matrix: np.ndarray, neighbors: list[np.ndarray]) -> np.ndarray:
    ...
    rows = np.concatenate(
        [np.full(len(adjacent), index, dtype=np.int64) for index, adjacent in enumerate(neighbors)]
    )
...
def compute_per_gene_metrics(
    ...
    columns: dict[str, np.ndarray] = {
        name: np.full(generated.n_vars, np.nan, dtype=np.float64)
        for name in (
            "pearson", "spearman", "generated_mean", "target_mean",
            "generated_variance", "target_variance", "generated_zero_prop",
            "target_zero_prop", "generated_moran_i", "target_moran_i",
        )
    }
    for start in range(0, generated.n_vars, int(chunk_size)):
        ...
        columns["spearman"][start:stop] = _pearson_columns(
            rankdata(simulated, method="average", axis=0),
            rankdata(observed, method="average", axis=0),
        )
...
def panel_summary(per_gene: pd.DataFrame) -> dict[str, float]:
    return {
        "mean_corr": safe_pearson(
            np.log1p(per_gene["generated_mean"]), np.log1p(per_gene["target_mean"]),
        ),
        ...
        "zero_ks": float(
            ks_2samp(per_gene["generated_zero_prop"], per_gene["target_zero_prop"]).statistic
        ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--generation-dir",
        type=Path,
        default=STUDY_DIR / "outputs" / "final",
    )
    ...
    if not (
        int(config["scoring"]["variance_ddof"]) == 0
        and config["scoring"]["zero_definition"]
        == "counts_less_than_or_equal_to_zero"
    ):
    ...
    expected_h5ads = {
        expected_job_path(args.generation_dir, job) / "generated.h5ad" for job in jobs
    }
    actual_h5ads = (
        set(args.generation_dir.rglob("generated.h5ad"))
        if args.generation_dir.exists()
        else set()
    )
    ...
    validation_rows = [
        validate_job(
            expected_job_path(args.generation_dir, job),
            job,
            config,
            paths,
            panels,
    ...
    if len(validation_rows) != int(config["expected_jobs"]) or not all(
        row["valid"] for row in validation_rows
    ):
    ...
    work_dir = Path(
        tempfile.mkdtemp(prefix=f"{config['scoring']['configuration_id']}-", dir=work_root)
    )
    ...
    for job in jobs:
        ...
        generation_provenance = json.loads(
            (candidate_dir / "provenance.json").read_text(encoding="utf-8")
        )
        ...
        neighbors = neighbor_indices(
            target.obsm["spatial"],
            int(config["scoring"]["neighbors_including_self"]),
        )
        per_gene = compute_per_gene_metrics(
            generated,
            target,
            neighbors,
            int(config["scoring"]["gene_chunk_size"]),
        )
        ...
        per_gene.insert(
            0,
            "reference_observed",
            ~per_gene["gene"].astype(str).isin(zero_genes),
        )
        primary = bool(
            np.isclose(
        ...
        observed_summary = {
            f"observed_{name}": value for name, value in panel_summary(observed).items()
        }
        summary_rows.append(
            {
                "job_id": job_id(config, job),
        ...
        support_rows.append(
            {
                ...
                "assignment_randomness": job.assignment_randomness,
                "donor_stratum": donor_stratum(job),
                "source_candidate_spots": support["source_candidate_spots"],
                ...
                "target_retained_fraction": support["target_retained_fraction"],
                "min_source_spots_per_label": support["min_source_spots_per_label"],
                "eligible_label_count": len(support["eligible_labels"]),
                "eligible_labels_json": json.dumps(support["eligible_labels"]),
                "source_label_counts_json": json.dumps(
                    support["source_label_counts"], sort_keys=True
                ),
                "excluded_target_label_counts_json": json.dumps(
                    support["excluded_target_label_counts"], sort_keys=True
                ),
                "dataset_gene_panel_count": support["dataset_gene_panel_count"],
                "reference_observed_gene_count": support[
                    "reference_observed_gene_count"
                ],
                "reference_zero_gene_count": support["reference_zero_gene_count"],
                "reference_zero_gene_sha256": support["reference_zero_gene_sha256"],
                "mask_axis": split.get("axis", ""),
                "mask_threshold": split.get("threshold", ""),
                "support_sha256": generation_provenance["support_sha256"],
            }
        )
        key = job_id(config, job)
        generation_inputs[key] = {
            "generated_sha256": sha256_file(generated_path),
            "generation_provenance_sha256": sha256_file(
    ...
    provenance = {
        ...
        "generation_validation": {
            "validated_jobs": len(validation_rows),
            "validator_sha256": sha256_file(STUDY_DIR / "validate.py"),
        },
        "sources": {
            "scorer_sha256": sha256_file(Path(__file__)),
            "workflow_sha256": sha256_file(STUDY_DIR / "workflow.py"),
        },
        "metric_contract": config["scoring"],
        "stratification_contract": STRATIFICATION_CONTRACT,
        "environment": {
            "python": sys.version,
        ...
        "outputs": {
            "per_gene_metrics.csv": sha256_file(per_gene_path),
            "summary.csv": sha256_file(summary_path),
            "support_audit.csv": sha256_file(support_path),
        },
    }

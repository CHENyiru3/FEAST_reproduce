...
def main() -> int:
    ...
    rows = []
    spatial_by_slice: dict[
        str, tuple[pd.Index, np.ndarray, np.ndarray, np.ndarray]
    ] = {}
    for job_index, row in enumerate(manifest.itertuples(index=False), start=1):
        ...
        if panel_hash != row.output_sha256:
            raise RuntimeError(f"fixed-panel checksum mismatch: {panel_path}")
        ...
        if row.slice_id not in spatial_by_slice:
            ...
            expected_names, expected_coordinates, distances, neighbors = spatial_by_slice[
                row.slice_id
            ]
            if not data.obs_names.equals(expected_names) or not np.array_equal(
        ...
        for method in selected_methods:
            ...
            if run.input_sha256 != panel_hash:
                raise RuntimeError(f"method input hash mismatch: {output}")
            if sha256(cluster_path) != run.clusters_sha256 or sha256(metadata_path) != run.metadata_sha256 or sha256(result_path) != run.result_h5ad_sha256:
            ...
            if method == "Leiden_unsupervised":
                ...
                label_informed_fields = {
                    "ari", "nmi", "ami", "ground_truth",
                    "adjusted_rand_score", "adjusted_mutual_info_score",
                    "normalized_mutual_info_score",
                }
                found = {
                    key
                    for item in diagnostics.get("sweep", [])
                    for key in item
                    if key.casefold() in label_informed_fields
                }
                if found:
    ...
    provenance = {
        ...
        "row_count": len(table),
        "selection_uses_ground_truth_for_leiden": False,
        "scoring_source": str(Path(__file__).resolve()),
        "scoring_source_sha256": sha256(Path(__file__).resolve()),
        "fixed_panel_manifest_sha256": sha256(args.panel_dir / "fixed_panel_manifest.csv"),
        "method_run_manifest_sha256": {
            method: sha256(args.method_dir / method / "run_manifest.csv")
            for method in selected_methods
        },
        "outputs": {
            path.name: sha256(path)
            for path in (table_path, summary_path, differences_path)
        },
    }

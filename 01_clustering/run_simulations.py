...
def main() -> int:
    ...
    if args.resume and manifest_path.is_file():
        previous_table = pd.read_csv(manifest_path, dtype={'slice_id': str, 'simulation_id': str})
    ...
    for slice_id, source in sources.items():
        ...
        for alteration in alterations:
            ...
            if args.resume and old.get('status') == 'ok' and (old.get('configuration_id') == config['configuration_id']) and (old.get('public_seed') == config['public_seed']) and (old.get('feast_version') == '1.0.2') and (old.get('feast_commit') == config['required_feast_commit']) and (old.get('spatial_mode') == 'reference_rank') and (old.get('assignment_solver') == 'scipy') and (old.get('assignment_blocks') in (False, 'False')) and output.is_file():
                rows.append(old)
                pd.DataFrame(rows).to_csv(manifest_path, index=False)
                print('  skipped verified', flush=True)
                continue
            if output.exists():
                failure = args.output_dir / 'failures' / slice_id / f"{alteration['simulation_id']}__{int(time.time())}.h5ad"
                failure.parent.mkdir(parents=True, exist_ok=True)
                shutil.move(str(output), str(failure))
            try:
                simulated = FEAST.simulate(reference, seed=int(config['public_seed']), alteration=alteration['alteration'], parameter_mode=feast_config['parameter_mode'], spatial_mode='reference_rank', annotation_key=config['annotation_key'], assignment_method=feast_config['assignment_method'], assignment_solver='scipy', assignment_blocks=False, use_distributional_alteration=True, ppf_method=feast_config['ppf_method'], beta_n_jobs=int(feast_config['beta_n_jobs']), beta_early_stopping_patience=int(feast_config['beta_early_stopping_patience']), convert_n_jobs=int(feast_config['convert_n_jobs']), n_jobs=int(feast_config['n_jobs']), use_heuristic_search=False, clip_overshoot_factor=0.0, boundary_multiplier=1.1, verbose=True)
            ...
            rows.append({'configuration_id': config['configuration_id'], 'slice_id': slice_id, 'simulation_id': alteration['simulation_id'], 'alteration_type': alteration['alteration_type'], 'alteration_value': alteration['value'], 'file_path': relative.as_posix(), 'public_seed': config['public_seed'], 'feast_version': FEAST.__version__, 'feast_commit': config['required_feast_commit'], 'spatial_mode': 'reference_rank', 'assignment_solver': 'scipy', 'assignment_blocks': False, 'solver_status': 'completed' if status == 'ok' else 'failed', 'elapsed_seconds': round(time.time() - started, 2), 'status': status})
            pd.DataFrame(rows).to_csv(manifest_path, index=False)

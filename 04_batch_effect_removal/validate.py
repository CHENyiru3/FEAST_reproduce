...
VALIDATOR_EXPECTED_QUERY_ZERO_IDS = {
    ("diagonal_affine", "1.25"): ["AGAAGAGCGCCGTTCC-1"],
...
STAMP_RETRY_POLICY_SHA256 = (
    "b4466eb3dd50eaa793dcfcc5a63ecfa4c7989271b7d28d995a6344ab37bdb40b"
)
STAMP_RETRY_ELIGIBLE = {
    ("shift_only", "1.50"),
    ("diagonal_affine", "1.50"),
}
STAMP_RETRY_UNCHANGED_CONTRACT = {
    "public_seed": 42,
    "n_topics": 10,
    "n_layers": 1,
    "hidden_size": 128,
...
METRIC_REPORT_FILES = (
    "atomic_metrics.csv",
    "primary_summary.csv",
    "common_support_sensitivity.csv",
    "graphst_pca20_vs_pca10_sensitivity.csv",
    "support_audit.csv",
    "old_vs_new_atomic_metrics.csv",
    "old_vs_new_simulation_hashes.csv",
    "historical_composite_disposition.csv",
    "summary.json",
)
...
def validator_sha256_json(value: object) -> str:
    payload = json.dumps(value, sort_keys=True, separators=(",", ":"), ensure_ascii=True)
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()


def validator_sha256_named_ids(identifiers: list[str]) -> str:
    payload = "\n".join(identifiers) + ("\n" if identifiers else "")
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()


def retry_bound_file(relative_path: object, expected_sha256: object, label: str) -> Path:
    ...
    check(
        path.is_relative_to(REPRODUCTION_ROOT),
        f"STAMP retry {label} path escapes the reproduction repository",
    )
    check(path.is_file(), f"STAMP retry {label} is missing: {path}")
    check(
        workflow.sha256_file(path) == str(expected_sha256),
        f"STAMP retry {label} hash changed: {path}",
    )
    return path
...
def validate_stamp_retry(metadata: dict, candidate: Path, mode: str, alpha: float) -> None:
    ...
    check(
        (mode, alpha_key) in STAMP_RETRY_ELIGIBLE,
        f"STAMP retry is not eligible for {mode}/alpha_{alpha_key}",
    )
    eligible_condition = retry.get("eligible_condition", {})
    check(
        eligible_condition.get("mode") == mode
        and alpha_text(eligible_condition.get("alpha", float("nan"))) == alpha_key,
        "STAMP retry eligible-condition record changed",
    )
    check(
        retry.get("policy_configuration_id") == STAMP_RETRY_POLICY_ID,
        "STAMP retry policy configuration ID changed",
    )
    check(retry.get("policy_path") == STAMP_RETRY_POLICY_PATH, "STAMP retry policy path changed")
    check(
        retry.get("policy_sha256") == STAMP_RETRY_POLICY_SHA256,
        "STAMP retry policy hash changed",
    )
    policy_path = retry_bound_file(
        retry.get("policy_path"), retry.get("policy_sha256"), "policy"
    )
    check(
        policy_path == (STUDY_ROOT / "stamp_numerical_retry_v1.yaml").resolve(),
        "STAMP retry policy resolves to the wrong file",
    )
    check(
        retry.get("changed_parameter")
        == {"name": "patience", "standard_value": 20, "retry_value": 10},
        "STAMP retry changed-parameter record is not patience 20 -> 10",
    )
    check(
        retry.get("unchanged_contract") == STAMP_RETRY_UNCHANGED_CONTRACT,
        "STAMP retry unchanged-contract record changed",
    )
    reason = str(retry.get("activation_reason", ""))
    check(
        all(token in reason for token in ("non-finite", "Pyro", "patience")),
        "STAMP retry activation reason is not explicit",
    )

    failure = retry_bound_file(
        retry.get("original_failure_path"),
        retry.get("original_failure_sha256"),
        "original failure",
    )
    disposition = retry_bound_file(
        retry.get("original_disposition_path"),
        retry.get("original_disposition_sha256"),
        "original disposition",
    )
    expected_failure_prefix = f"STAMP__{mode}__alpha_{alpha_key}__"
    run_dir = candidate.parents[3].resolve()
    check(
        candidate.name == f"alpha_{alpha_key}"
        and candidate.parent.name == mode
        and candidate.parent.parent.name == "STAMP"
        and failure.name == "FAILED.txt"
        and disposition.name == "DISPOSITION.txt"
        and failure.parent == disposition.parent
        and failure.parent.name.startswith(expected_failure_prefix)
        and failure.parent.parent.resolve() == run_dir / "failures",
        "STAMP retry is not bound to its matching preserved failure",
    )
    failure_text = failure.read_text(encoding="utf-8")
    check(
        all(token in failure_text for token in ("caux", "tensor([nan]", "cuda:0")),
        "STAMP retry failure does not prove the declared Pyro non-finiteness",
    )
    check(
        disposition.read_text(encoding="utf-8").strip()
        == "exit=1; missing metadata.json",
        "STAMP retry failure disposition changed",
    )

    actual_contract = {
        "public_seed": int(metadata.get("seed", -1)),
        "n_topics": int(metadata.get("n_topics", -1)),
        "n_layers": int(metadata.get("n_layers", -1)),
        "hidden_size": int(metadata.get("hidden_size", -1)),
        "mode": metadata.get("mode"),
        "learning_rate": float(metadata.get("learning_rate", float("nan"))),
        "max_epochs": int(metadata.get("max_epochs", -1)),
        "min_epochs": int(metadata.get("min_epochs", -1)),
        "device": metadata.get("device"),
        "zero_library_strategy": metadata.get("zero_library_safeguard", {}).get(
            "strategy"
        ),
    }
    check(
        actual_contract == STAMP_RETRY_UNCHANGED_CONTRACT,
        "STAMP retry candidate differs from its unchanged contract",
    )
    provenance = metadata.get("provenance", {})
    context = provenance.get("candidate_context", {})
    check(
        context.get("method") == "STAMP"
        and context.get("deformation_mode") == mode
        and alpha_text(context.get("alpha", float("nan"))) == alpha_key,
        "STAMP retry candidate context changed",
    )
    ...
    check(
        float(diagnostics.get("learning_rate", float("nan"))) == 0.005
        and diagnostics.get("cuda_execution_verified") is True
        and diagnostics.get("actual_device") == "cuda:0",
        "STAMP retry LR/CUDA provenance changed",
    )
    ...
    graph_contract = {
        ...
        "n_spots": 7222,
        "n_batches": 2,
        "directed_edge_count": 43332,
        "cross_batch_edge_count": 0,
        "self_edge_count": 0,
    }
    check(
        all(graph.get(key) == value for key, value in graph_contract.items()),
        "STAMP retry graph contract changed",
    )
    matrix_contract = metadata.get("fixed_panel_input_matrices", {})
    for key in ("reference_count_matrix_sha256", "query_count_matrix_sha256"):
        check(
            bool(matrix_contract.get(key)) and matrix_contract.get(key) == diagnostics.get(key),
            f"STAMP retry data-lineage hash changed: {key}",
        )
    expected_inputs = {
        (run_dir / "simulations" / mode / "alpha_0.00.h5ad").resolve(),
        (run_dir / "simulations" / mode / f"alpha_{alpha_key}.h5ad").resolve(),
        (run_dir / "panel" / "panel.csv").resolve(),
        (run_dir / "panel" / "provenance.json").resolve(),
    }
    observed_inputs = {
    ...
    policy_sources = [
        ...
        if Path(str(item.get("path", ""))).resolve() == policy_path
    ]
    check(
        len(policy_sources) == 1
        and policy_sources[0].get("sha256") == STAMP_RETRY_POLICY_SHA256,
        "STAMP retry provenance does not bind the pinned policy source",
    )
    training = metadata.get("training", {})
    check(
        training.get("early_stopped") is True
        and int(training.get("early_stopping_counter", -1)) == 10,
        "STAMP retry did not stop under the declared patience-10 rule",
    )
...
def validate_provenance_records(records: list[dict], collection: str) -> None:
    ...
    for record in records:
        ...
        check(
            workflow.sha256_file(path) == record.get("sha256"),
            f"provenance hash mismatch: {path}",
        )
...
def validator_local_batch_entropy(
...
def validator_paired_retrieval(
    values: np.ndarray,
    n_reference: int,
) -> tuple[float, float]:
...
def validator_mmd2_rbf_biased(
    ...
    reference_index = rng.choice(
        n_reference,
        min(VALIDATOR_MMD_SAMPLE_PER_BATCH, n_reference),
        replace=False,
    )
    query_index = rng.choice(
        query_count,
        min(VALIDATOR_MMD_SAMPLE_PER_BATCH, query_count),
        replace=False,
    )
    ...
    value = (
        rbf_kernel(reference, reference, gamma=gamma).mean()
        + rbf_kernel(query, query, gamma=gamma).mean()
        - 2.0 * rbf_kernel(reference, query, gamma=gamma).mean()
    )
    return float(max(value, 0.0)), bandwidth


def validator_paired_expression_correlation(
    matrix: np.ndarray,
    n_reference: int,
    paired_spot_ids: np.ndarray,
) -> float:
    ...
    if zero_variance.any():
        ...
        raise ValueError(
            "paired-expression correlation has zero-variance named pairs: "
            f"{names[:20]}"
        )
...
def validator_representations(
    ...
    return [
        ...
        (
            "pca10_randomized_seed42_sensitivity",
            False,
...
def validator_metric_row(
    *,
    ...
    return {
        ...
        "batch_mixing_entropy_k30": validator_local_batch_entropy(batch_labels, neighbors),
        ...
        "domain_purity_k30": validator_domain_purity(domain_text, valid_domain, neighbors),
...
def validator_load_support(
    ...
    *,
    ...
    try:
        ...
        check(
            not any(
                gene not in reference.var_names or gene not in query.var_names
                for gene in genes
            ),
        ...
        check(
            not reference_zero_ids,
            f"reference fixed panel has raw-zero rows for {mode}/{alpha:.2f}",
        )
        expected_query_zero_ids = VALIDATOR_EXPECTED_QUERY_ZERO_IDS.get(
            (mode, alpha_text(alpha)), []
        )
        check(
            query_zero_ids == expected_query_zero_ids,
            f"query fixed-panel raw-zero IDs changed for {mode}/{alpha:.2f}",
        )
        return {
            "n_spots": int(reference.n_obs),
            "spot_ids": reference.obs_names.astype(str).to_numpy(copy=True),
            "remove_pairs": query_zero,
            "query_zero_count": int(query_zero.sum()),
            "query_zero_ids": query_zero_ids,
            "query_zero_ids_sha256": validator_sha256_named_ids(query_zero_ids),
            "reference_zero_count": 0,
            "reference_zero_ids": reference_zero_ids,
            "reference_zero_ids_sha256": validator_sha256_named_ids(reference_zero_ids),
...
def validator_row_set(
    *,
    ...
    if method == "scVI":
        expression_correlation = validator_paired_expression_correlation(
            candidate["normalized"][keep],
            n_reference,
            paired_spot_ids,
        )
    removed_count = (
        int(support["query_zero_count"])
        if support_variant == VALIDATOR_PAIRED_SUPPORT
        else 0
    )
    ...
    for representation, primary, values in validator_representations(method, embedding, seed):
        ...
        row.update(
            {
                "configuration_id": f"{candidate_id.split('__', 1)[0]}__atomic_metrics",
                "study_id": "04",
                "legacy_study_id": "08",
                "mode": mode,
                ...
                "primary_support": support_variant == VALIDATOR_ALL_SPOTS,
                "analysis_role": (
                    "primary"
                    if support_variant == VALIDATOR_ALL_SPOTS and primary
                    else "representation_sensitivity"
                    if support_variant == VALIDATOR_ALL_SPOTS
                    else "paired_support_sensitivity"
                    if primary
                    else "representation_and_paired_support_sensitivity"
                ),
                "removed_paired_spots": removed_count,
                "removed_named_pairs": json.dumps(removed_ids, ensure_ascii=True),
                "removed_named_pairs_sha256": validator_sha256_named_ids(removed_ids),
                "fixed_panel_zero_query_rows": int(support["query_zero_count"]),
                "fixed_panel_zero_query_spot_ids": json.dumps(
                    support["query_zero_ids"], ensure_ascii=True
                ),
                "fixed_panel_zero_query_spot_ids_sha256": support[
                    "query_zero_ids_sha256"
                ],
                "fixed_panel_zero_reference_rows": int(support["reference_zero_count"]),
                "fixed_panel_zero_reference_spot_ids": json.dumps(
                    support["reference_zero_ids"], ensure_ascii=True
                ),
                "fixed_panel_zero_reference_spot_ids_sha256": support[
                    "reference_zero_ids_sha256"
                ],
                "n_reference": n_reference,
                "n_query": n_reference,
                ...
                "candidate_solver_diagnostics_sha256": candidate[
                    "solver_diagnostics_sha256"
                ],
                "candidate_method_environment_json": candidate["environment_json"],
                "candidate_method_environment_sha256": candidate["environment_sha256"],
                "candidate_artifact_hashes_json": candidate["artifact_hashes_json"],
                "candidate_artifact_hashes_sha256": candidate["artifact_hashes_sha256"],
                "input_artifact_ids_json": candidate["input_artifact_ids_json"],
            }
        )
        rows.append(row)
    return rows
...
def validate_scvi(metadata: dict, result: ad.AnnData, candidate: Path) -> None:
    ...
    check(
        seed_application.get("lightning_deterministic") is True,
        "scVI deterministic Lightning control changed",
    )
    raw_checks = metadata.get("raw_count_checks", [])
    check(len(raw_checks) == 2, "scVI raw-count audit must contain reference and query")
    check(
        {str(item.get("label")) for item in raw_checks} == {"reference", "query"},
        "scVI raw-count audit labels changed",
    )
    ...
    check(
        metadata.get("normalized_expression_transform_batch") == "ref",
        "scVI decoded expression was not normalized under the common reference batch",
    )
    check(
        metadata.get("accelerator_requested") in {"gpu", "cuda"},
        "scVI did not request GPU execution",
    )
    check(
        str(metadata.get("actual_device", "")).startswith("cuda"),
        "scVI metadata does not prove CUDA execution",
    )
    check(
        diagnostics.get("cuda_execution_verified") is True
        and str(diagnostics.get("actual_device", "")).startswith("cuda"),
        "scVI provenance does not prove CUDA execution",
    )
    for key in (
        "all_history_values_finite",
        "latent_all_finite",
        "normalized_expression_all_finite",
        "normalized_expression_nonnegative",
    ...
    for name, item in history_summary.items():
        ...
        check(
            int(item.get("finite_values", -1)) == int(item.get("total_values", -2)),
            f"scVI history contains non-finite values: {name}",
        )
    for key in (
    ...
    check(
        np.equal(raw_values, np.floor(raw_values)).all(),
        "scVI raw-count layer is non-integral",
    )
    fixed_panel_matrices = metadata.get("fixed_panel_input_matrices", {})
    check(
        fixed_panel_matrices.get("canonical_representation")
        == "study04-count-matrix-v1 int64 row-major",
        "scVI fixed-panel raw-count representation changed",
    )
    for key in ("reference_count_matrix_sha256", "query_count_matrix_sha256"):
        check(
            fixed_panel_matrices.get(key) == diagnostics.get(key),
            f"scVI raw-count lineage hash changed: {key}",
        )

    check(
        environment.get("numpy_supported_by_feast") is False,
        "scVI external NumPy limitation is not explicit",
    )
    ...
    check(
        "FEAST is not imported or executed" in limitation,
        "scVI NumPy limitation does not isolate the external worker from FEAST",
    )
    check(
        environment.get("feast_numpy_support_range") == ">=1.24,<2",
        "scVI FEAST NumPy support range changed",
    )
    numpy_version = str(environment.get("numpy", ""))
    check(
        np.lib.NumpyVersion(numpy_version) >= np.lib.NumpyVersion("2.0.0"),
        "scVI external environment no longer matches its recorded NumPy-2 limitation",
    )
    check((candidate / "model" / "model.pt").is_file(), "scVI model artifact is missing")


def recompute_support_audit(
    run_dir: Path,
    config: dict,
    genes: list[str],
) -> pd.DataFrame:
    ...
    for mode in config["simulation"]["modes"]:
        ...
        for raw_alpha in config["simulation"]["alpha_levels"]:
            ...
            support = validator_load_support(
                reference_path,
                query_path,
                genes,
                mode=mode,
            ...
            rows.append(
                {
                    "mode": mode,
                    "alpha": alpha,
                    "reference_path": support["reference_path"],
                    "reference_sha256": support["reference_sha256"],
                    "query_path": support["query_path"],
                    "query_sha256": support["query_sha256"],
                    "n_spots_per_batch": int(support["n_spots"]),
                    "fixed_panel_zero_reference_rows": int(support["reference_zero_count"]),
                    "fixed_panel_zero_reference_spot_ids": json.dumps(
                        support["reference_zero_ids"], ensure_ascii=True
                    ),
                    ...
                    "all_spots_removed_paired_spots": 0,
                    "paired_support_removed_paired_spots": int(
                        support["query_zero_count"]
                    ),
                    "paired_support_removed_named_pairs": json.dumps(
                        support["query_zero_ids"], ensure_ascii=True
                    ),
                    "paired_support_removed_named_pairs_sha256": support[
                        "query_zero_ids_sha256"
                    ],
                    "paired_support_n_reference": int(support["n_spots"])
                    - int(support["query_zero_count"]),
                    "paired_support_n_query": int(support["n_spots"])
                    - int(support["query_zero_count"]),
                }
            )
...
def main() -> int:
    ...
    expected_cells = {
        (str(mode), alpha_text(alpha))
        for mode in config["simulation"]["modes"]
        for alpha in config["simulation"]["alpha_levels"]
    }
    observed_cells = {
        (str(row.mode), alpha_text(row.alpha)) for row in manifest.itertuples(index=False)
    }
    ...
    check(
        simulation_provenance.get("configuration_id") == config["configuration_id"],
        "simulation configuration ID changed",
    )
    check(
    ...
    check(
        int(simulation_provenance.get("public_seed", -1)) == int(config["public_seed"]),
        "simulation public seed changed",
    )
    check(
        simulation_provenance.get("feast", {}).get("commit")
        == config["required_feast_commit"],
        "simulation FEAST commit changed",
    )
    check(
    ...
    baseline_matrix_hashes: set[str] = set()
    ...
    check(
        panel_provenance.get("configuration_id") == f"{config['configuration_id']}__fixed_panel",
        "panel configuration ID changed",
    )
    check(
    ...
    for mode in config["simulation"]["modes"]:
        ...
        for alpha_value in config["simulation"]["alpha_levels"]:
            ...
            if alpha == 0.0:
                continue
            alpha_key = alpha_text(alpha)
            support_by_cell[(mode, alpha_key)] = validator_load_support(
                reference_path,
                run_dir / "simulations" / mode / f"alpha_{alpha_key}.h5ad",
                genes,
                mode=mode,
                alpha=alpha,
            )
    declared_paired_cells = {
        (str(mode), alpha_text(alpha))
    ...
    for method in workflow.METHOD_ARTIFACTS:
        for mode in config["simulation"]["modes"]:
            for alpha_value in config["simulation"]["alpha_levels"]:
                ...
                check(
                    metadata.get("runtime_controls", {}).get("pip_check_status") == "ok",
                    f"{method} environment pip check was not successful",
                )
                method_environment = (
                ...
                if numpy_disposition is False:
                    check(
                        "unsupported"
                        in str(method_environment.get("external_environment_limitation", "")),
                ...
                check(
                    np.array_equal(result.obs["batch"].astype(str).to_numpy(), expected_batches),
                    "method result batch order changed",
                )
                check(
                    np.array_equal(
                        result.obs["source_spot_id"].astype(str).to_numpy(), expected_source_ids
                    ),
                    "method result named spot order changed",
                )
                ...
                check(
                    np.array_equal(result_embedding, saved_embedding),
                    f"{method} NPZ/H5AD embeddings differ",
                )
                ...
                normalized = (
                    np.asarray(result.layers["scvi_normalized"])
                    if method == "scVI"
                    else None
                )
                result.file.close()
                artifact_hashes = {
                    relative: workflow.sha256_file(candidate / relative)
                    for relative in workflow.METHOD_ARTIFACTS[method]
                }
                ...
                candidate_payload = {
                    "embedding": np.asarray(saved_embedding, dtype=np.float64),
                    "batch_labels": saved_batches,
                    "domain_labels": domain_labels,
                    "normalized": normalized,
                    "embedding_sha256": workflow.sha256_file(candidate / "embeddings.npz"),
                    "result_sha256": workflow.sha256_file(candidate / "result.h5ad"),
                    "metadata_sha256": workflow.sha256_file(metadata_path),
                    "provenance_sha256": validator_sha256_json(provenance),
                    "solver_diagnostics_sha256": validator_sha256_json(diagnostics),
                    "environment_json": json.dumps(
                        method_environment, sort_keys=True, separators=(",", ":")
                    ),
                    "environment_sha256": validator_sha256_json(method_environment),
                    "artifact_hashes_json": json.dumps(
                        artifact_hashes, sort_keys=True, separators=(",", ":")
                    ),
                    "artifact_hashes_sha256": validator_sha256_json(artifact_hashes),
                    "input_artifact_ids_json": json.dumps(input_artifact_ids),
                }
                alpha_key = alpha_text(alpha)
                support = support_by_cell[(mode, alpha_key)]
                recomputed_metric_rows.extend(
                    validator_row_set(
                        method=method,
                        mode=mode,
                        alpha=alpha,
                        ...
                        support=support,
                        support_variant=VALIDATOR_ALL_SPOTS,
                        keep=np.ones(2 * n_spots, dtype=bool),
                ...
                if (mode, alpha_key) in declared_paired_cells:
                    ...
                    recomputed_metric_rows.extend(
                        validator_row_set(
                            ...
                            candidate=candidate_payload,
                            support=support,
                            support_variant=VALIDATOR_PAIRED_SUPPORT,
                            keep=np.concatenate([~remove, ~remove]),
                            seed=int(config["public_seed"]),
                        )
                    )
    ...
    expected_method_cells = {
        (method, str(mode), alpha_text(alpha))
        for method in workflow.METHOD_ARTIFACTS
        for mode in config["simulation"]["modes"]
        for alpha in config["simulation"]["alpha_levels"]
        if float(alpha) != 0.0
    ...
    observed_method_cells = {
        (str(row.method), str(row.mode), alpha_text(row.alpha))
    ...
    check(
        ...
        "method manifest contains an unsuccessful job",
    )
    for row in run_manifest.itertuples(index=False):
        alpha_key = alpha_text(row.alpha)
        expected_candidate_id = (
            f"{config['configuration_id']}__{row.method}__{row.mode}__alpha_{alpha_key}"
        )
        expected_candidate_path = (
            run_dir / "methods" / str(row.method) / str(row.mode) / f"alpha_{alpha_key}"
        ).resolve()
        check(str(row.candidate_id) == expected_candidate_id, "method manifest candidate ID changed")
        check(
            Path(str(row.output_dir)).resolve() == expected_candidate_path,
            "method manifest accepted-output path changed",
        )
        ...
        if pd.notna(log_value):
            ...
            check(
                workflow.sha256_file(log_path) == str(getattr(row, "log_sha256")),
                f"method manifest log hash mismatch: {log_path}",
            )
    ...
    recomputed_metrics = pd.DataFrame(recomputed_metric_rows).sort_values(
        ["method", "mode", "alpha", "support_variant", "representation"]
    ).reset_index(drop=True)
    check(len(recomputed_metrics) == 60, "validator recomputed metric row count changed")
    check(
        set(recomputed_metrics.columns) == set(metrics.columns),
        "validator-owned atomic metric schema differs from atomic_metrics.csv",
    )
    ...
    recomputed_old_vs_new = historical.build_old_vs_new(
        metrics,
        historical_atomic_path=historical_atomic_path,
        expected_keys=scoring.expected_keys(config),
        metric_columns=scoring.METRIC_COLUMNS,
    )
    ...
    recomputed_simulation_comparison = historical.build_simulation_hash_comparison(
        manifest,
        historical_atomic_path=historical_atomic_path,
        modes=config["simulation"]["modes"],
        alpha_levels=config["simulation"]["alpha_levels"],
        expected_keys=scoring.expected_keys(config),
    )
    observed_simulation_comparison = pd.read_csv(
        metric_paths["old_vs_new_simulation_hashes.csv"]
    )
    assert_frame_matches(
        observed_simulation_comparison,
        recomputed_simulation_comparison,
        "old-versus-new simulation hashes",
    )
    check(
        observed_simulation_comparison["numerical_change_class"]
        .eq(historical.NUMERICAL_CHANGE_CLASS)
        .all(),
        "fresh simulation hashes were not classified as RNG/API repair plus full rerun",
    )
    check(
        observed_simulation_comparison["interpretation"]
        .eq("cross_stage_rng_api_repair_and_full_rerun_not_repeat_drift_evidence")
        .all(),
        "fresh simulation hashes were misclassified as repeat-drift evidence",
    )
    recomputed_composite = historical.invalid_composite_disposition(
        invalid_composite_path,
        metric_paths["atomic_metrics.csv"],
    )
    observed_composite = pd.read_csv(metric_paths["historical_composite_disposition.csv"])
    assert_frame_matches(
        observed_composite,
        recomputed_composite,
        "historical composite disposition",
    )
    check(
        observed_composite["numeric_comparison_performed"].eq(False).all(),
        "invalid historical composite was compared numerically",
    )
    ...
    check(
        metric_summary.get("graphst_pca20_vs_pca10_sensitivity_rows") == 15,
        "GraphST PCA sensitivity count changed",
    )
    check(metric_summary.get("old_vs_new_comparable_rows") == 60, "old/new row count changed")
    check(
        metric_summary.get("old_vs_new_atomic_metrics_per_row") == 10,
        "old/new atomic metric count changed",
    )
    check(
        metric_summary.get("old_vs_new_simulation_hash_rows") == 14,
        "old/new simulation hash row count changed",
    )
    ...
    check(
        metric_summary.get("numeric_delta_disposition") == historical.CLAIM_DISPOSITION,
        "numeric deltas were promoted beyond author-review evidence",
    )
    check(
        metric_summary.get("historical_comparable_atomic_metrics")
        == {
            "path": str(historical_atomic_path),
            "sha256": historical.HISTORICAL_ATOMIC_SHA256,
        },
        "metric summary historical atomic-table binding changed",
    )
    check(
        metric_summary.get("historical_invalid_composite")
        == {
            "path": str(invalid_composite_path),
            "sha256": historical.INVALID_COMPOSITE_SHA256,
            "numeric_comparison_performed": False,
        },
        "metric summary invalid-composite binding changed",
    )

    check(
        metric_provenance.get("configuration_id")
        == f"{config['configuration_id']}__atomic_metrics",
        "metric provenance configuration ID changed",
    )
    ...
    check(
        metric_provenance.get("diagnostics", {}).get("reference_fixed_panel_zero_rows") == 0,
        "metric provenance reference-zero assertion changed",
    )
    check(
        metric_provenance.get("diagnostics", {}).get("active_manuscript_claim") is None,
        "metric provenance binds an undeclared manuscript claim",
    )
    ...
    check(
        source_roles
        == {"scorer", "historical_comparison", "workflow_dispatcher", "configuration"},
        "metric source-lineage roles changed",
    )
    input_by_role = {}
    ...
    for role in (
        ...
        "fixed_panel",
        "fixed_panel_provenance",
        "method_run_manifest",
        "historical_comparable_atomic_metrics",
        "historical_invalid_composite_disposition_only",
    ):
        check(len(input_by_role.get(role, [])) == 1, f"metric provenance role changed: {role}")
    expected_historical_inputs = {
        record["role"]: record
        for record in historical.pinned_input_records(
            historical_atomic_path,
            invalid_composite_path,
        )
    }
    for role, expected_record in expected_historical_inputs.items():
        check(
            input_by_role[role] == [expected_record],
            f"metric provenance historical input binding changed: {role}",
        )
    check(len(input_by_role.get("simulation_h5ad", [])) == 14, "metric provenance lacks 14 simulations")
    candidate_input_count = sum(
        len(input_by_role.get(f"{method}_candidate_artifact", []))
        for method in workflow.METHOD_ARTIFACTS
    )
    expected_candidate_artifacts = sum(
        12 * len(artifacts) for artifacts in workflow.METHOD_ARTIFACTS.values()
    )
    check(
        candidate_input_count == expected_candidate_artifacts,
        "metric provenance does not bind every method artifact",
    )
    expected_metric_output_paths = {path.resolve() for path in metric_paths.values()}
    observed_metric_output_paths = {
        Path(record["path"]).resolve() for record in metric_provenance["outputs"]
    }
    check(
        observed_metric_output_paths == expected_metric_output_paths,
        "metric provenance output set changed",
    )
    ...
    summary = {
        ...
        "method_manifest_verified_success_rows": len(run_manifest),
        "method_counts": {"GraphST": 12, "STAMP": 12, "scVI": 12},
        "atomic_metric_rows": len(metrics),
        "old_vs_new_comparable_rows": len(observed_old_vs_new),
        "old_vs_new_simulation_hash_rows": len(observed_simulation_comparison),
        "graphst_pca_sensitivity_rows": len(observed_pca),
        "reference_fixed_panel_zero_rows": 0,
        "query_zero_paired_support_counts": {
            "diagonal_affine/1.25": 1,
            "diagonal_affine/1.50": 3,
            "shift_only/1.50": 2,
        },
        "cross_batch_edges": 0,
        "active_manuscript_claim": None,
        "exit_2_guard_triggered": False,
        "decision": "validated_fresh_supplementary_candidate_author_review_required",
        "limitations": [
            "GraphST, STAMP, and scVI external environments do not expand FEAST support.",
            "GraphST and scVI explicitly use NumPy versions outside FEAST's supported range.",
            "Alpha values above 1.00 are extrapolation evidence.",
            "The 15-row GraphST PCA-20/PCA-10 sensitivity must accompany future use.",
            "Numeric old-versus-new deltas are author-review evidence because no active manuscript claim is bound.",
    ...
    provenance = {
        ...
        "sources": [
            {
                ...
                "sha256": workflow.sha256_file(Path(__file__).resolve()),
            },
            {
                "path": str((STUDY_ROOT / "score.py").resolve()),
                "sha256": workflow.sha256_file(STUDY_ROOT / "score.py"),
            },
            {
                "path": str((STUDY_ROOT / "compare_historical.py").resolve()),
                "sha256": workflow.sha256_file(STUDY_ROOT / "compare_historical.py"),
            },
            {
                "path": str(config["_config_path"]),
                "sha256": workflow.sha256_file(config["_config_path"]),
            },
        ],
        "inputs": [
            ...
            *historical.pinned_input_records(
                historical_atomic_path,
                invalid_composite_path,
            ),
        ],
        ...
        "diagnostics": {
            ...
            "metric_rows": 60,
            "old_vs_new_rows": 60,
            "old_vs_new_simulation_hash_rows": 14,
            "graphst_pca_sensitivity_rows": 15,
            "reference_fixed_panel_zero_rows": 0,
            "scvi_cuda_training_decode_and_numpy_limitation_validated": True,
            "active_manuscript_claim": None,
            "exit_2_guard_triggered": False,
        },
    }

...
def add_joint_pca(reference: ad.AnnData, moving: ad.AnnData, n_pcs: int):
    ...
    combined = ad.concat(
        [reference, moving], label="slice", keys=["reference", "moving"],
...
def install_solver_gate(config: dict):
    ...
    def strict_emd(
        a, b, M, numItermax=100000, log=False, center_dual=True,
        numThreads=1, check_marginals=True, **kwargs,
    ):
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            transport, diagnostics = original_emd(
                a, b, M, numItermax=numItermax, log=True,
                center_dual=center_dual, numThreads=numThreads,
                check_marginals=check_marginals, **kwargs,
            )
        row = {
            "call_index": len(state["inner_emd_calls"]) + 1,
            "num_iter_max": int(numItermax),
            "result_code": int(diagnostics.get("result_code", -1)),
            "solver_warning": diagnostics.get("warning"),
            "caught_warnings": [str(item.message) for item in caught],
            "cost": float(diagnostics.get("cost", np.nan)),
        }
        row["positive_optimality_evidence"] = bool(
            row["result_code"] == 1
            and row["solver_warning"] is None
            and not row["caught_warnings"]
    ...
    def strict_cg(a, b, M, reg, f, df, G0=None, line_search=None, **kwargs):
        ...
        relative_delta = (
            float(absolute_delta / abs(losses[-1])) if losses[-1] != 0 else np.inf
        )
        ...
        positive = bool(
            ...
            and (
                ...
                or absolute_delta < absolute_tolerance
            )
        )
        state["outer_conditional_gradient"] = {
            "iterations": iterations,
            "num_iter_max": max_iterations,
            "final_loss": float(losses[-1]),
            "final_absolute_loss_delta": absolute_delta,
            "final_relative_loss_delta": relative_delta,
            "relative_tolerance": relative_tolerance,
            "absolute_tolerance": absolute_tolerance,
            "positive_convergence_evidence": positive,
        }
        if not positive:
...
def main() -> int:
    ...
    try:
        ...
        reference, moving, used_pcs, common_genes = add_joint_pca(
            reference, moving, int(config["n_pcs"])
        )
        state, restore = install_solver_gate(config)
        try:
            with warnings.catch_warnings(record=True) as caught:
                warnings.simplefilter("always")
                coupling, objective = pst.pairwise_align(
                    reference, moving,
                    alpha=float(config["alpha"]),
                    dissimilarity=str(config["dissimilarity"]),
                    use_rep="X_pca",
                    norm=bool(config["normalize_spatial_distances"]),
                    numItermax=int(config["outer_max_iterations"]),
                    return_obj=True,
                    verbose=False,
                    gpu_verbose=False,
                )
        ...
        dangerous = [
            message for message in escaped
            if "numItermax" in message or "optimal" in message.lower()
            or "numerical" in message.lower()
        ]
        if dangerous:
        ...
        if not state["inner_emd_calls"] or not all(
            row["positive_optimality_evidence"] for row in state["inner_emd_calls"]
        ):
        ...
        aligned_coords, rotation, rotation_angle, ref_center, mov_center = weighted_procrustes(
            reference_coords, moving_coords, coupling
        )
        ...
        pd.DataFrame(
            {
                ...
                "x_moving": moving_coords[:, 0], "y_moving": moving_coords[:, 1],
                "x_aligned": aligned_coords[:, 0], "y_aligned": aligned_coords[:, 1],
            }
        ).to_csv(coordinates_path, index=False)
        pd.DataFrame(state["inner_emd_calls"]).to_csv(
            args.output_dir / "inner_emd_diagnostics.csv", index=False
        )
        diagnostics = {
            "status": "converged",
            "positive_convergence_evidence": True,
            "inner_emd_calls": len(state["inner_emd_calls"]),
            "all_inner_emd_optimal": True,
            "outer_conditional_gradient": outer,
            "uncaptured_warnings": escaped,
            "coupling": {
                "shape": list(coupling.shape), "mass": mass,
                "max_row_marginal_error": row_error,
                "max_column_marginal_error": column_error,
        ...
        transform = {
            ...
            "method": "paste", "status": "ok", "canonical_candidate": True,
            "public_seed": args.seed,
            "runtime_seconds": round(time.monotonic() - started, 3),
            "feast": {
                "version": moving_raw.uns["feast_reproduce"]["feast_version"],
                "commit": moving_raw.uns["feast_reproduce"]["feast_commit"],
            },
            "parameters": config, "n_pcs_used": used_pcs,
            "common_gene_count": common_genes,
            "rotation_angle": rotation_angle,
            "rotation_matrix": rotation.tolist(),
            "reference_center": ref_center.tolist(), "moving_center": mov_center.tolist(),
            "input_artifacts": {
            ...
            "environment": {
                ...
                "paste_bio": importlib.metadata.version("paste-bio"),
                "pot": importlib.metadata.version("POT"),
            ...
            "output_artifacts": {
                ...
                for path in (aligned_path, coupling_path, coordinates_path)
            },

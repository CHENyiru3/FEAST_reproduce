...
EXTENSION_REPORT_DIR = REPOSITORY_ROOT / '01_clustering' / 'outputs' / 'expanded_mean_variance_fc_0_20_5_20260806' / 'report'
...
EXTENSION_CONDITIONS = {('mean_0_20', 'mean', 0.2), ('mean_5', 'mean', 5.0), ('variance_0_20', 'variance', 0.2), ('variance_5', 'variance', 5.0)}
METRICS = ('ARI', 'NMI', 'AMI')
METHOD_ORDER = ('GraphST', 'STAGATE_mclust', 'Leiden_unsupervised')
METHOD_LABELS = {'GraphST': 'GraphST', 'STAGATE_mclust': 'STAGATE + mclust', 'Leiden_unsupervised': 'Label-free Leiden'}
METHOD_COLORS = {'GraphST': '#0072B2', 'STAGATE_mclust': '#E69F00', 'Leiden_unsupervised': '#009E73'}
METHOD_MARKERS = {'GraphST': 'o', 'STAGATE_mclust': 's', 'Leiden_unsupervised': '^'}
ORDERED_ALTERATIONS = ('mean', 'variance', 'sparsity')
ALTERATION_LABELS = {'mean': 'Mean', 'variance': 'Variance', 'sparsity': 'Sparsity'}
_ALT_PREFIX_MAP = [('sparsity_neg', 'sparsity'), ('sparsity_pos', 'sparsity'), ('variance', 'variance'), ('mean', 'mean')]
BASELINE_REF = 'real_baseline'
FEAST_BASELINE = 'baseline'
NEUTRAL_LEVEL = {'mean': 1.0, 'variance': 1.0, 'sparsity': 0.0}

def repository_path(path: Path) -> str:
    return str(path.resolve().relative_to(REPOSITORY_ROOT))

def registered_sources() -> tuple[Path, Path, dict[str, object]]:
    ...
    return (metrics_path, decision_path, candidate)
...
def validated_extension_sources() -> dict[str, object]:
    if not EXTENSION_VALIDATION_PATH.is_file():
        raise FileNotFoundError(f'Expanded Study 01 validation is missing: {EXTENSION_VALIDATION_PATH}')
    ...
    if int(validation.get('simulation_jobs', -1)) != 12 or int(validation.get('fixed_panel_jobs', -1)) != 12 or int(validation.get('metric_rows', -1)) != 36 or (sum((int(value) for value in validation.get('method_jobs', {}).values())) != 36):
        raise ValueError('Expanded Study 01 validation counts do not match the extension contract.')
    verify_registered_file(EXTENSION_METRICS_PATH)
    verify_registered_file(EXTENSION_DIAGNOSTICS_PATH)
    return {'metrics_path': EXTENSION_METRICS_PATH, 'diagnostics_path': EXTENSION_DIAGNOSTICS_PATH, 'validation_path': EXTENSION_VALIDATION_PATH}
...
def load_alteration_data(metrics_path: Path, extension_metrics_path: Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, pd.DataFrame]]:
    ...
    conditions = {(str(row.simulation_id), str(row.alteration_type), float(row.alteration_value)) for row in extension[['simulation_id', 'alteration_type', 'alteration_value']].drop_duplicates().itertuples(index=False)}
    ...
    baseline_ref = metrics.loc[metrics['simulation_id'] == BASELINE_REF, ['slice_id', 'method', *METRICS]].copy()
    baseline_ref = baseline_ref.rename(columns={m: f'{m}_ref' for m in METRICS})
    feast_baseline = metrics.loc[metrics['simulation_id'] == FEAST_BASELINE, ['slice_id', 'method', *METRICS]].copy()
    alteration_rows = metrics.loc[~metrics['simulation_id'].isin([BASELINE_REF, FEAST_BASELINE])].copy()
    ...
    alteration_rows['level'] = alteration_rows.apply(lambda r: _signed_level(r['simulation_id'], r['plot_group'], r['raw_level']), axis=1)
...
def configure_matplotlib() -> None:
    plt.rcParams.update({'font.family': 'DejaVu Sans', 'font.size': 10.5, 'axes.facecolor': 'white', 'figure.facecolor': 'white', 'savefig.facecolor': 'white', 'axes.spines.top': False, 'axes.spines.right': False, 'pdf.fonttype': 42, 'ps.fonttype': 42, 'svg.fonttype': 'none'})

def _panel_y_range(alteration_data: dict[str, pd.DataFrame], baseline_ref: pd.DataFrame, metric: str) -> tuple[float, float]:
...
def draw_ordered_panel(ax: plt.Axes, data: pd.DataFrame, metric: str, y_lim: tuple[float, float], neutral_level: float, x_label: str, show_xlabel: bool) -> None:
    ...
    for method in METHOD_ORDER:
        ...
        for _slice_id, slice_data in method_data.groupby('slice_id', sort=True):
            ...
            ax.plot(slice_data['level_num'], slice_data[metric], color=color, linewidth=0.85, alpha=0.28, zorder=1)
        ...
        if len(valid) >= 2:
            ...
            ax.plot(x_vals, y_vals, color=color, linewidth=2.0, marker=marker, markersize=5.8, markeredgecolor='white', markeredgewidth=0.7, zorder=3)
    ax.set_ylim(*y_lim)
    ...
    if fold_change_axis:
        ...
        ax.set_xlim(min(levels) - 0.06 * (max(levels) - min(levels) or 1.0), max(levels) + 0.06 * (max(levels) - min(levels) or 1.0))
...
def draw_figure(alteration_data: dict[str, pd.DataFrame], baseline_ref: pd.DataFrame, output_dir: Path, dpi: int) -> list[Path]:
    ...
    for row, metric in enumerate(METRICS):
        ...
        for col, alt_type in enumerate(panel_types):
            ...
            if data is None or data.empty:
                ax.text(0.5, 0.5, 'No data', transform=ax.transAxes, ha='center', va='center', fontsize=9, color='#999999')
                ax.set_xticks([])
                ax.set_yticks([])
                continue
            show_xlabel = row == n_rows - 1
            x_label = 'Fold change (neutral = 1)' if alt_type in {'mean', 'variance'} else 'Signed logit shift (neutral = 0)'
            draw_ordered_panel(ax, data, metric, y_lim, NEUTRAL_LEVEL[alt_type], x_label, show_xlabel)
            if col == 0:
                ax.set_ylabel(f'{metric} ↑', fontsize=11.5, fontweight='bold', labelpad=5)
            if row == 0:
                ax.set_title(ALTERATION_LABELS.get(alt_type, alt_type), fontsize=12.5, fontweight='bold', pad=10)
    method_handles = []
    for method in METHOD_ORDER:
        method_handles.append(plt.Line2D([0], [0], marker=METHOD_MARKERS[method], linestyle='-', markerfacecolor=METHOD_COLORS[method], markeredgecolor='white', color=METHOD_COLORS[method], markersize=7.5, linewidth=1.8, label=METHOD_LABELS[method]))
    fig.legend(handles=method_handles, loc='upper center', bbox_to_anchor=(0.76, 0.985), ncol=3, frameon=False, fontsize=10, handlelength=1.2, handletextpad=0.6, columnspacing=1.5)
    fig.text(0.075, 0.985, 'Clustering sensitivity to controlled alterations', ha='left', va='center', fontsize=13.5, fontweight='bold')
    fig.text(0.075, 0.945, 'Thin traces: individual DLPFC slices; solid lines and markers: three-slice means. Mean/variance axes show requested FC on a log scale.', ha='left', va='center', fontsize=8.9, color='#666666')
    fig.subplots_adjust(left=0.085, right=0.995, top=0.9, bottom=0.09, hspace=0.34, wspace=0.25)
...
def write_provenance(output_dir: Path, outputs: list[Path], metrics_path: Path, decision_path: Path, extension_sources: dict[str, object], baseline_ref: pd.DataFrame, alteration_rows: pd.DataFrame) -> Path:
    payload = {'schema_version': 4, 'figure_set': 'study01_alteration_sensitivity', 'source': {'publication_manifest': repository_path(MANIFEST_PATH), 'metrics_csv': repository_path(metrics_path), 'publication_decision': repository_path(decision_path), 'expanded_metrics_csv': repository_path(extension_sources['metrics_path']), 'expanded_diagnostics_csv': repository_path(extension_sources['diagnostics_path']), 'expanded_validation': repository_path(extension_sources['validation_path']), 'plot_script': repository_path(Path(__file__))}, 'design': {'metrics': list(METRICS), 'methods': list(METHOD_ORDER), 'ordered_alterations': list(ORDERED_ALTERATIONS), 'baseline_reference': BASELINE_REF, 'sparsity_merged': True, 'var_hetero_removed': True, 'combined_removed': True, 'shared_y_per_metric': True, 'mean_variance_x_scale': 'log', 'mean_variance_nominal_levels_added': [0.2, 5.0], 'mean_variance_levels_are_requested_not_realized': True, 'slice_variation_encoding': 'thin individual-slice traces; solid three-slice mean traces', 'neutral_setting_encoding': 'dashed vertical line', 'raw_slice_reference_lines_drawn': False, 'slice_ids': sorted(baseline_ref['slice_id'].unique().tolist()), 'n_slices': int(baseline_ref['slice_id'].nunique()), 'n_alteration_rows': int(len(alteration_rows)), 'n_expanded_alteration_rows': int(alteration_rows['source_set'].eq('expanded_extremes').sum()), 'winner_claimed': False}, 'scientific_disposition': {'figure_promotion_authorized': False, 'aggregate_method_ranking_authorized': False, 'winner_claim_authorized': False, 'supported_interpretation': 'Clustering sensitivity across ordered requested alteration levels, including the validated mean and variance extreme-level extension. Nominal fold changes must be interpreted with the realized diagnostics.'}, 'render_environment': {'matplotlib': matplotlib.__version__, 'numpy': np.__version__, 'pandas': pd.__version__}, 'outputs': {path.name: None for path in sorted(outputs)}}
...
def main() -> int:
    args = parse_args()
    metrics_path, decision_path, candidate = registered_sources()
    verify_registered_file(metrics_path)
    verify_registered_file(decision_path)
    ...
    baseline_ref, alteration_rows, alteration_data = load_alteration_data(metrics_path, extension_sources['metrics_path'])
    ...
    provenance_path = write_provenance(args.output_dir, outputs, metrics_path, decision_path, extension_sources, baseline_ref, alteration_rows)

...
def read_score_layer(path: Path, label: str, expected_keys: set[tuple[str, str, float]]) -> pd.DataFrame:
    ...
    if len(frame) != 12 or frame.duplicated(list(KEY_COLUMNS)).any() or score_keys(frame) != expected_keys or (set(frame['method'].astype(str)) != {'rctd', 'cell2location'}) or (set(frame['status'].astype(str)) != {'ok'}):
...
def build_old_vs_new(historical: pd.DataFrame, prior: pd.DataFrame, fresh: pd.DataFrame) -> pd.DataFrame:
    ...
    layers = {'historical': historical, 'prior_repaired': prior, 'fresh': fresh}
...
def main() -> int:
    ...
    if len(simulations) != 6 or set(simulations['pair_id'].astype(str)) != expected_pairs or simulations['pair_id'].duplicated().any() or (set(simulations['status'].astype(str)) != {'ok'}) or (len(methods) != 12) or methods[['pair_id', 'method']].duplicated().any() or (set(methods['status'].astype(str)) != {'ok'}) or (set(zip(methods['pair_id'].astype(str), methods['method'].astype(str))) != {(pair_id, method) for pair_id in expected_pairs for method in ('rctd', 'cell2location')}) or (set(simulations['feast_commit'].astype(str)) != {str(config['required_feast_commit'])}) or (set(methods['feast_commit'].astype(str)) != {str(config['required_feast_commit'])}) or (set(methods['feast_version'].astype(str)) != set(simulations['feast_version'].astype(str))) or (set(simulations['public_seed'].astype(int)) != {int(config['seed'])}) or (set(methods['public_seed'].astype(int)) != {int(config['seed'])}) or (not all((str(row.pair_id) == f'{str(row.slice_id)}__resolution_{float(row.resolution):g}' for row in simulations.itertuples(index=False)))) or (not all((str(row.pair_id) == f'{str(row.slice_id)}__resolution_{float(row.resolution):g}' for row in methods.itertuples(index=False)))):
    ...
    for simulation_row in simulations.itertuples(index=False):
        ...
        if embedded.get('feast_commit') != simulation_row.feast_commit or int(embedded.get('public_seed', -1)) != int(simulation_row.public_seed):
        ...
        if truth_values.shape != truth.shape or not np.allclose(truth_values, truth.to_numpy(), atol=1e-12, rtol=0):
        ...
        for method in ('rctd', 'cell2location'):
            ...
            if str(method_row.slice_id) != str(simulation_row.slice_id) or not np.isclose(float(method_row.resolution), float(simulation_row.resolution), atol=0, rtol=0) or Path(method_row.simulation_path).resolve() != simulation_path.resolve() or (Path(method_row.reference_path).resolve() != reference_path.resolve()) or (str(method_row.feast_commit) != str(simulation_row.feast_commit)) or (str(method_row.feast_version) != str(simulation_row.feast_version)) or (int(method_row.public_seed) != int(simulation_row.public_seed)):
                raise RuntimeError(f'{method} manifest lineage differs: {pair_id}')
            metadata = json.loads(metadata_path.read_text())
            if metadata.get('status') != 'validated_success' or int(metadata.get('public_seed', -1)) != int(simulation_row.public_seed):
            ...
            method_lineage[method] = {'prediction': artifact(output_path, f'{pair_id}:{method}:prediction'), 'metadata': artifact(metadata_path, f'{pair_id}:{method}:metadata'), 'configuration_id': str(method_row.configuration_id)}
        ...
        if truth_types != reference_types or truth_types != cell2location_types:
            raise RuntimeError(f'truth/reference/Cell2location full type sets differ: {pair_id}')
        ...
        if rctd.to_numpy().min(initial=0) < -1e-06:
        ...
        if np.max(np.abs(rctd.sum(axis=1).to_numpy() - 1), initial=0) > 1e-05:
            raise RuntimeError('RCTD rows do not sum to one')
        if np.max(np.abs(cell2location.loc[positive_spots].sum(axis=1).to_numpy() - 1), initial=0) > 1e-06:
        ...
        for method, prediction in (('rctd', rctd), ('cell2location', cell2location.loc[positive_spots])):
            result = metrics(truth_scored, with_other(prediction, common))
            rows.append({'method': method, 'pair_id': pair_id, 'slice_id': str(simulation_row.slice_id), 'resolution': float(simulation_row.resolution), 'status': 'ok', 'comparison_scope': 'same_expression_exact_spots_common_named_types_plus_other', 'n_input_spots': len(full_spots), 'n_scored_spots': len(positive_spots), 'n_zero_library_spots_excluded': len(zero_spots), 'n_common_named_cell_types': len(common), 'n_scored_cell_types_including_other': len(common) + 1, **result})
        support_rows.append({'pair_id': pair_id, 'slice_id': str(simulation_row.slice_id), 'resolution': float(simulation_row.resolution), 'exact_positive_spot_support_between_methods': True, 'cell2location_full_spot_support_exact': True, 'n_full_spots': len(full_spots), 'n_scored_spots': len(positive_spots), 'n_zero_library_spots': len(zero_spots), 'zero_library_disposition': 'excluded_from_both_methods_scoring', 'n_truth_cell_types': len(truth.columns), 'n_reference_cell_types': len(reference_types), 'n_rctd_cell_types': len(rctd.columns), 'n_cell2location_cell_types': len(cell2location.columns), 'n_common_named_types': len(common), 'truth_type_set_equals_reference_type_set': True, 'truth_type_set_equals_cell2location_type_set': True, 'rctd_type_set_equals_truth_ordered_common_type_set': True, 'reference_common_type_set_equals_canonical_common_type_set': set(common).issubset(reference_types), 'cell2location_common_type_set_equals_canonical_common_type_set': set(common).issubset(cell2location_types), 'canonical_common_order_source': 'truth_columns', 'truth_cell_type_order_json': json.dumps(list(truth.columns)), 'canonical_common_cell_type_order_json': json.dumps(list(common)), 'rctd_raw_column_order_matches_canonical_common_order': rctd.columns.equals(common), 'rctd_raw_column_order_required': False, 'other_residual_included': True})
        scoring_input_lineage.append({'pair_id': pair_id, 'slice_id': str(simulation_row.slice_id), 'resolution': float(simulation_row.resolution), 'simulation': artifact(simulation_path, f'{pair_id}:simulation'), 'truth': artifact(truth_path, f'{pair_id}:truth'), 'reference': artifact(reference_path, f'{pair_id}:reference'), 'methods': method_lineage})
    scores = pd.DataFrame(rows)
    if len(scores) != 12 or scores.duplicated(['method', 'pair_id']).any() or (not np.isfinite(scores.loc[:, SCORE_METRICS].to_numpy(np.float64)).all()):
    ...
    status = 'stop_headline_direction_changed' if direction_changed else 'candidate_direction_unchanged_pending_author_review'
    decision = {'status': status, 'historical_direction_audit_path': str(args.direction_audit.resolve()), 'historical_metric_winners': historical_metric_winners, 'metric_winners': metric_winners, 'direction_tolerance': DIRECTION_TOLERANCE, 'metric_differences': differences, 'changed_metric_directions': changed_metric_directions, 'direction_changed': direction_changed, 'aggregate_winner_ranking_authorized': False, 'decision_rule': 'stop if any atomic metric direction changes, ties, or is indeterminate; otherwise require author review', 'publication_claim_authorized': False, 'claim': 'Fresh common-support results preserve the historical per-metric direction vector; author review is still required before publication.' if not direction_changed else 'At least one fresh common-support metric changes or cannot resolve its historical direction; stop and report before publication integration.'}
    ...
    provenance = {'schema_version': 3, 'configuration_id': 'study03-common-support-score-v3', 'generated_at_utc': datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z'), 'public_seed': int(config['seed']), 'config': artifact(args.config, 'study03:config'), 'scorer': artifact(Path(__file__), 'study03:common-support-scorer'), 'feast_versions': sorted(set(simulations['feast_version'].astype(str))), 'feast_commits': sorted(set(simulations['feast_commit'].astype(str))), 'run_lineage': {'simulation_manifest': artifact(simulation_manifest_path, 'study03:simulation-manifest'), 'method_manifest': artifact(method_manifest_path, 'study03:method-manifest'), 'simulation_configuration_ids': sorted(set(simulations['configuration_id'].astype(str))), 'method_configuration_ids': sorted(set(methods['configuration_id'].astype(str)))}, 'historical_inputs': {'article_historical_scores': artifact(args.historical_scores, 'study03:article-historical-scores'), 'prior_repaired_scores': artifact(args.prior_scores, 'study03:prior-repaired-scores'), 'historical_direction_audit': artifact(args.direction_audit, 'study03:historical-direction-audit')}, 'scoring_input_lineage': scoring_input_lineage, 'numerical_change_policy': {'delta_definition': 'target_minus_source', 'change_tolerance': CHANGE_TOLERANCE, 'classifications': CHANGE_CLASSIFICATIONS, 'interpretation': 'All cross-layer deltas mix workflow changes and are not attributed to a single scientific or engineering cause.'}, 'outputs': {path.name: None for path in (scores_path, summary_path, support_path, old_vs_new_path, decision_path)}}

...
def collect_candidates(config: dict, age: str) -> dict[int, tuple[Path, dict]]:
    ...
    unrecorded = [
        path for path in age_root.glob("*/*.h5ad")
        if path.resolve() not in declared_paths and not path.name.endswith(".tmp.h5ad")
    ]
    if unrecorded:
...
def consolidate_age(config: dict, age: str) -> None:
    ...
    try:
        for index in range(len(entries)):
            ...
            if workflow.sha256_file(source) != str(declared["output_sha256"]):
                raise ValueError(f"shard hash mismatch: {source}")
            ...
            workflow.validate_publication_lineage(
                result, config, age, entries[index], transport_contract, reference_artifacts
            )
            ...
            rows.append({
                **summary,
                ...
                "filename": filename,
                "public_seed": int(config["public_seed"]) + index,
                "assignment_randomness": float(provenance["assignment_randomness"]),
                "matrix_sha256": workflow.matrix_sha256(result.X),
                "output_sha256": workflow.sha256_file(destination),
            })
        ...
        metadata = {
            ...
            "n_spots": int(manifest["n_spots"].sum()),
            "z_start": float(manifest.iloc[0]["z_world"]),
            "z_end": float(manifest.iloc[-1]["z_world"]),
            "config_sha256": workflow.sha256_file(config["_config_path"]),
            "blueprint_sha256": workflow.sha256_file(workflow.blueprint_path(config, age)),
            "blueprint_manifest_sha256": workflow.sha256_file(workflow.blueprint_manifest_path(config, age)),
            "reference_input_sha256": {path.name: workflow.sha256_file(path) for path in workflow.reference_paths(config, age)},
            "reference_artifacts": reference_artifacts,
            "run_py_sha256": workflow.sha256_file(workflow.STUDY_ROOT / "run.py"),
            "consolidate_py_sha256": workflow.sha256_file(Path(__file__).resolve()),
            "transport_config": transport_contract,
            "feast": feast,
            "smoothing": "none",
            "all_transport_records_converged": True,
        }

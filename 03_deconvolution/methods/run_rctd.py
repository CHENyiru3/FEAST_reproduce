...
def main() -> int:
    ...
    try:
        ...
        retained_types = label_counts[
            label_counts >= int(config["min_cells_per_type"])
        ].index
        ...
        with tempfile.TemporaryDirectory(prefix="rctd_pub_") as temporary:
            work = Path(temporary)
            pd.DataFrame(
                spatial_counts.T, index=spatial.var_names, columns=full_spots
            ).to_csv(work / "counts.csv")
            coordinates = np.asarray(spatial.obsm["spatial"], dtype=np.float64)
            pd.DataFrame(
                {"x": coordinates[:, 0], "y": coordinates[:, 1], "nUMI": spatial_counts.sum(axis=1)},
                index=full_spots,
            ).to_csv(work / "coordinates.csv")
            ref_columns = [f"ref_{index}" for index in range(int(keep.sum()))]
            pd.DataFrame(
                reference_counts[keep].T, index=reference.var_names, columns=ref_columns
            ).to_csv(work / "reference.csv")
            pd.Series(
                [type_to_code[value] for value in retained_labels],
                index=ref_columns, name="cell_type",
            ).to_csv(work / "cell_types.csv")
            ...
            command = [
                str(args.rscript), str(Path(__file__).with_name("run_rctd_backend.R")),
                "--counts", str(work / "counts.csv"),
                "--coords", str(work / "coordinates.csv"),
                "--reference", str(work / "reference.csv"),
                "--cell-types", str(work / "cell_types.csv"),
                "--output", str(args.output),
                "--cache-dir", str(config["cache_dir"]),
                "--max-cores", str(config["max_cores"]),
                "--seed", str(args.seed),
            ]
            cache_dir = str(config["cache_dir"])
            environment = {
                **os.environ,
                "RCTD_CACHE_DIR": cache_dir,
        ...
        if not np.isfinite(values).all() or values.min(initial=0) < -1e-6:
            raise RuntimeError("RCTD output contains invalid weights")
        if np.max(np.abs(values.sum(axis=1) - 1), initial=0) > 1e-5:
        ...
        }.issubset(runtime_versions):
            raise RuntimeError("RCTD backend did not report its runtime versions")
        metadata = {
            "status": "validated_success", "method": "rctd", "mode": str(config["mode"]),
            "public_seed": args.seed, "input_path": str(args.input.resolve()),
            ...
            "stdout_path": str(stdout_path.resolve()), "stdout_sha256": sha256_file(stdout_path),
            "stderr_path": str(stderr_path.resolve()), "stderr_sha256": sha256_file(stderr_path),
            "n_input_spots": spatial.n_obs, "n_output_spots": len(prediction),
            "n_zero_library_spots_excluded": int(spatial.n_obs - len(prediction)),
            "n_retained_cell_types": len(original_types),
            "min_cells_per_type": int(config["min_cells_per_type"]),
            "rng_policy": "R set.seed(public_seed) before createRctd/runRctd",
            "environment": runtime_versions,
            "solver_diagnostics": {
                "backend_return_code": completed.returncode,
                "weights_all_finite": True,
                "weights_nonnegative_with_tolerance": True,
                "weights_rows_normalized": True,
                "mode": str(config["mode"]),
                "convergence_claim": False,
                "disposition": (
                    "spacexr returned a completed full-mode weights assay; the API "
                    "does not expose a separate positive convergence flag"
                ),
            },
        }
        ...
        metadata_path.write_text(
            json.dumps(
                {"status": "failed_noncanonical", "method": "rctd", "error": repr(exc),

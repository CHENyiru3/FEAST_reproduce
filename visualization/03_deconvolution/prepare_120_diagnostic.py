...
CANONICAL_TYPES = (
    ...
    "26 P GABA", "27 MY GABA", "28 CB GABA", "29 CB Glut", "30 Astro-Epen",
    "31 OPC-Oligo", "32 OEC", "33 Vascular", "34 Immune",
)

# Fixed palette supplied for the reference-faithful 30-class rendering.
PALETTE = {
    "01 IT-ET Glut": "#436B92", "02 NP-CT-L6b Glut": "#A0B3CC",
    "03 OB-CR Glut": "#D77A47", "04 DG-IMN Glut": "#E6AE80",
    "05 OB-IMN GABA": "#58914E", "06 CTX-CGE GABA": "#95C28C",
    "07 CTX-MGE GABA": "#A83E3C", "08 CNU-MGE GABA": "#DC8E8B",
    "09 CNU-LGE GABA": "#786397", "10 LSX GABA": "#AD9FBB",
    "11 CNU-HYa GABA": "#73544E", "12 HY GABA": "#AA8E88",
    "13 CNU-HYa Glut": "#B2739E", "14 HY Glut": "#DBA8BC",
    "18 TH Glut": "#8A8A89", "19 MB Glut": "#C6C7C6",
    "20 MB GABA": "#A7AA55", "21 MB Dopa": "#C9CA90",
    "22 MB-HB Sero": "#5CADB2", "23 P Glut": "#9BC9CB",
    "24 MY Glut": "#D2D561", "26 P GABA": "#C1B598",
    "27 MY GABA": "#8BBF93", "28 CB GABA": "#4B4484",
    "29 CB Glut": "#AB719D", "30 Astro-Epen": "#68AF78",
    "31 OPC-Oligo": "#57676E", "32 OEC": "#74848B",
    "33 Vascular": "#8AB164", "34 Immune": "#D8E09F",
}
...
def checked_path(path_text: str, expected_sha: str, label: str) -> Path:
    path = Path(path_text)
    if not path.is_file():
        raise FileNotFoundError(f"missing {label}: {path}")
    observed = sha256(path)
    if observed != expected_sha:
        raise ValueError(f"SHA-256 mismatch for {label}: {observed} != {expected_sha}")
    return path
...
def configure_matplotlib() -> None:
    plt.rcParams.update({
        ...
        "ps.fonttype": 42,
        "svg.fonttype": "none",
        "svg.hashsalt": "feast-study03-120-preflight",
    })


def canonical_proportions(
    table: pd.DataFrame, fine_to_class: dict[str, str], expected_index: pd.Index,
) -> pd.DataFrame:
...
def select_roi(
    ...
    for center_x in candidates_x:
        for center_y in candidates_y:
            mask = (
                (xy[:, 0] >= center_x - width / 2)
                & (xy[:, 0] <= center_x + width / 2)
                & (xy[:, 1] >= center_y - height / 2)
                & (xy[:, 1] <= center_y + height / 2)
            )
            if mask.sum() < 25:
                continue
    ...
    return bounds + ({
        "selection": "max_local_composition_entropy_times_log_spot_count_on_x0.10_ground_truth",
        "x0.10_spot_count": n_spots,
        "mean_composition_entropy_nats": entropy,
...
def main() -> int:
    ...
    for resolution in RESOLUTIONS:
        matches = manifest.loc[
            (manifest["slice_id"] == "120")
            & np.isclose(manifest["resolution"].astype(float), float(resolution))
            & (manifest["status"] == "ok")
        ]
        if len(matches) != 1:
    ...
    try:
        ...
        mapping = pd.DataFrame({
            "fine": reference.obs["ground_truth"].astype(str),
            "display": reference.obs["class"].astype(str),
        }).drop_duplicates()
    finally:
    ...
    for resolution, row in rows.items():
        sim_path = checked_path(row["simulation_path"], row["simulation_sha256"], f"x{resolution} simulation")
        truth_path = checked_path(row["truth_path"], row["truth_sha256"], f"x{resolution} ground truth")
        ...
        if not np.allclose(canonical_sums[positive], 1.0, atol=1e-8):
        ...
        for display_order, cell_type in enumerate(CANONICAL_TYPES, start=1):
            average_rows.append({
                ...
                "cell_type": cell_type,
                "mean_proportion_nonempty_locations": float(mean[cell_type]),
                "present_in_reference": bool(cell_type in set(reference_classes)),
            })
        summaries.append({
            "resolution": resolution,
            "n_spatial_locations": int(len(xy)),
            "n_nonempty_locations": int(positive.sum()),
            "n_empty_locations": int((~positive).sum()),
            "source_cells": int(source_cells.sum()),
            "mean_source_cells_per_location": float(source_cells.mean()),
            "median_source_cells_per_location": float(np.median(source_cells)),
            "min_source_cells_per_location": int(source_cells.min()),
            "max_source_cells_per_location": int(source_cells.max()),
            "ground_truth_rows_equal_locations": True,
            "ground_truth_columns_are_30_canonical_order": True,
            "positive_row_sum_min": float(canonical_sums[positive].min()),
            "positive_row_sum_max": float(canonical_sums[positive].max()),
            "empty_row_sum_max": float(canonical_sums[~positive].max()) if (~positive).any() else 0.0,
        })
        simulations[resolution] = {
            "xy": xy, "canonical": canonical, "positive": positive,
            "simulation_path": sim_path, "truth_path": truth_path,
        }

    roi_x0, roi_x1, roi_y0, roi_y1, roi_details = select_roi(
        simulations["0.1"]["xy"], simulations["0.1"]["canonical"], x_limits, y_limits
    )
    roi = (roi_x0, roi_x1, roi_y0, roi_y1)
    for summary in summaries:
        resolution = str(summary["resolution"])
        xy = simulations[resolution]["xy"]
        summary["roi_spatial_location_count"] = int(
            ((xy[:, 0] >= roi_x0) & (xy[:, 0] <= roi_x1) & (xy[:, 1] >= roi_y0) & (xy[:, 1] <= roi_y1)).sum()
        )
    ...
    for cell_type in CANONICAL_TYPES:
        ...
        if mask.any():
            axes[0].scatter(
                reference_xy[mask, 0], reference_xy[mask, 1],
                s=0.32, c=PALETTE[cell_type], alpha=0.88, linewidths=0,
                rasterized=True,
            )
    ...
    for axis, resolution, panel in zip(axes[1:], RESOLUTIONS, ("B", "C"), strict=True):
        ...
        axis.add_patch(Rectangle(
            ...
            fill=False, edgecolor="#D9534F", linewidth=0.9, zorder=3,
        ))
        axis.set_title(
            f"{panel}  Spatial centres — resolution ×{resolution}",
            fontweight="semibold", pad=8,
        )
    fig.text(
        0.5, 0.015,
        "Pre-Cell2location diagnostic: blue = non-empty aggregate location; grey = empty location; red = fixed ROI.",
        ha="center", va="bottom", fontsize=9, color="#4D4D4D",
    )
    ...
    pd.DataFrame(summaries).sort_values("resolution", ascending=False).to_csv(
        OUTPUT_DIR / f"{OUTPUT_STEM}_summary.csv", index=False
    )
    pd.DataFrame(average_rows).to_csv(OUTPUT_DIR / f"{OUTPUT_STEM}_average_proportions.csv", index=False)
    (OUTPUT_DIR / f"{OUTPUT_STEM}_roi.json").write_text(json.dumps({
        "source_slice": "Zhuang-ABCA-1.120",
        "coordinate_system": "reference .obsm['spatial']; common limits shared by all later spatial panels",
        "x_min": roi_x0, "x_max": roi_x1, "y_min": roi_y0, "y_max": roi_y1,
        "selection_details": roi_details,
        "x_limits": x_limits, "y_limits": y_limits,
    }, indent=2) + "\n")
    (OUTPUT_DIR / f"{OUTPUT_STEM}_alignment.json").write_text(json.dumps({
        "canonical_order": list(CANONICAL_TYPES),
        "palette": PALETTE,
        "reference_observed_classes": sorted(set(reference_classes)),
        "canonical_type_count": len(CANONICAL_TYPES),
        "reference_observed_class_count": len(set(reference_classes)),
        "fine_label_count": len(fine_to_class),
        "all_reference_classes_canonical": True,
        "all_truth_tables_reindexed_to_canonical_order": True,
        "cell2location_status": "not_started_pending_preflight_review",
    }, indent=2) + "\n")
    provenance = {
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "stage": "pre_cell2location_diagnostic",
        "source_slice": "Zhuang-ABCA-1.120",
        "reference_path": str(REFERENCE.resolve()),
        "reference_sha256": sha256(REFERENCE),
        "simulation_run_dir": str(RUN_DIR.resolve()),
        "simulation_provenance_sha256": sha256(simulation_provenance_path),
        "simulation_manifest_sha256": sha256(simulation_manifest_path),
        "diagnostic_script_sha256": sha256(Path(__file__)),
        "inputs": {
            resolution: {
                "simulation": {"path": str(values["simulation_path"]), "sha256": sha256(values["simulation_path"])},
                "ground_truth": {"path": str(values["truth_path"]), "sha256": sha256(values["truth_path"])},
            }
            for resolution, values in simulations.items()
        },
        "outputs": {
            name: sha256(OUTPUT_DIR / name)
            for name in (
                f"{OUTPUT_STEM}.pdf", f"{OUTPUT_STEM}.png", f"{OUTPUT_STEM}.svg",
                f"{OUTPUT_STEM}_summary.csv", f"{OUTPUT_STEM}_average_proportions.csv",
                f"{OUTPUT_STEM}_roi.json", f"{OUTPUT_STEM}_alignment.json",
            )
        },
        "cell2location_started": False,
    }

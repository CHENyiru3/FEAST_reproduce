...
# Required by PyTorch for deterministic CUDA matrix multiplication.
...
def tree_sha256(root: Path) -> str:
    """Hash package files with relative paths for an install-level fingerprint."""
    digest = hashlib.sha256()
...
def generated_expression_by_cell(
...
def run(args: argparse.Namespace) -> None:
    ...
    # scCube requires a dense matrix and a cell identifier column.
    ...
    model = scCube.scCube()
    generated_meta, generated_data = model.train_vae_and_generate_cell(
        sc_adata=reference,
        cell_key="Cell",
        celltype_key=SOURCE_ID_KEY,
    ...
    identity_map = pd.DataFrame(
        {
            "source_reference_id": reference_ids,
            "sccube_generated_cell": cell_for_source.loc[reference_ids].to_numpy(),
            "spatial_x": spatial[:, 0],
            "spatial_y": spatial[:, 1],
        }
    )
    ...
    provenance = {
        ...
        "configuration_id": "study00-sccube-slideseq001-identity-v1",
        "method": "scCube",
        "method_version": package_version("scCube"),
        "method_package_tree_sha256": tree_sha256(Path(scCube.__file__).resolve().parent),
        "input_h5ad": str(args.input_h5ad.resolve()),
        "input_artifact_id": "study00-reference-Slideseq_001",
        "input_sha256": sha256(args.input_h5ad),
        ...
        "feast_version": args.feast_version,
        "feast_commit": feast_commit(args.feast_repo),
        "external_method_environment_supported_by_feast": False,
        "external_method_environment_note": (
            "External scCube environment; not the supported FEAST validation environment."
        ),
        "runner": str(script_path),
        "runner_sha256": sha256(script_path),
        "identity_map_csv": str(args.identity_map_csv.resolve()),
        "identity_map_sha256": sha256(args.identity_map_csv),
        "environment_json": str(args.environment_json.resolve()),
        "environment_sha256": sha256(args.environment_json),
        "reference_transfer": "unique_source_id_exact_coordinate_restore",
        "cell_key_requested": "ground_truth",
        "cell_key_used": SOURCE_ID_KEY,
        "source_label_count": int(reference.n_obs),
        "generated_values": "continuous nonnegative expression values",
        "solver_diagnostics": {
            "dense_ot_called": False,
            "generated_spots": int(output.n_obs),
            "generated_genes": int(output.n_vars),
            "unique_source_ids": int(output.obs["sccube_source_reference_id"].nunique()),
            "unique_spatial_coordinates": int(np.unique(output.obsm["spatial"], axis=0).shape[0]),
            "exact_reference_id_order": bool(output.obs_names.equals(reference_ids)),
            "exact_reference_gene_order": bool(output.var_names.equals(gene_names)),
            "exact_reference_spatial_order": bool(np.array_equal(output.obsm["spatial"], spatial)),
        },
        "matrix_diagnostics": {
            "format": "csr",
            ...
            "library_min": float(library_sizes.min()),
            "library_max": float(library_sizes.max()),
    ...
    provenance["output_sha256"] = sha256(args.output_h5ad)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--feast-repo",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "FEAST",
    )

...
def main() -> int:
    ...
    simulation_manifest = pd.read_csv(args.simulation_dir / 'simulation_manifest.csv', dtype={'slice_id': str})
    ...
    required_simulation_contract = simulation_manifest['configuration_id'].eq(config['configuration_id']).all() and simulation_manifest['public_seed'].eq(config['public_seed']).all() and simulation_manifest['feast_version'].astype(str).eq('1.0.2').all() and simulation_manifest['feast_commit'].eq(config['required_feast_commit']).all() and simulation_manifest['spatial_mode'].eq('reference_rank').all() and simulation_manifest['assignment_solver'].eq('scipy').all() and simulation_manifest['assignment_blocks'].eq(False).all()
    ...
    for slice_value in config['slices']:
        ...
        candidates = [(row.simulation_id, args.simulation_dir / row.file_path, row.alteration_type) for row in slice_rows.itertuples(index=False)]
        candidates.append(('real_baseline', raw_path, 'real_baseline'))
        for simulation_id, source, alteration_type in candidates:
            ...
            prepared.uns['publication_reproduction'] = {'configuration_id': config['configuration_id'], 'feast_version': '1.0.2', 'feast_commit': config['required_feast_commit'], 'public_seed': int(config['public_seed']), 'source_type': 'raw_real_baseline' if simulation_id == 'real_baseline' else 'fresh_feast_simulation', 'panel_selection': 'raw-derived Seurat-v3', 'preprocessing': f"normalize_total(target_sum={settings['target_sum']}) then log1p"}
            ...
            rows.append({'slice_id': slice_id, 'simulation_id': simulation_id, 'alteration_type': alteration_type, 'file_path': relative.as_posix(), 'n_spots': prepared.n_obs, 'n_genes': prepared.n_vars, 'status': 'ok'})

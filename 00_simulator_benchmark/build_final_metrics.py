...
from score import pair_metrics  # noqa: E402
...
def main() -> None:
    ...
    if sha256(REFERENCE) != rerun_provenance["input_sha256"]:
        raise ValueError("scCube rerun reference hash changed")
    if sha256(CANDIDATE) != rerun_provenance["output_sha256"]:
        raise ValueError("scCube rerun output hash changed")
    ...
    for column in table.columns:
        ...
        if before_missing != after_missing or before != after:
            changes.append(
                {
                    "field": column,
                    "before": None if before_missing else json_scalar(before),
                    "after": None if after_missing else json_scalar(after),
                }
            )
    ...
    write_json(
        support_path,
        {
            "simulator": "scCube",
            "sample": "Slideseq_001",
            "reference_spots": int(reference.n_obs),
            "candidate_spots": int(candidate.n_obs),
            "exact_spot_order": True,
            "exact_gene_order": True,
            "exact_spatial_order": True,
            "paired_spots": int(metrics["n_paired_spots"]),
            "spot_pairing": metrics["spot_pairing"],
            "identity_flag": bool(metrics["identity_flag"]),
        },
    )
    ...
    write_json(
        decision_path,
        {
            "status": "validated_external_rerun_author_review_required",
            "publication_claim_authorized": False,
            "figure_promotion_authorized": False,
            "winner_or_composite_authorized": False,
            "feast_aggregate_metrics_worsened": ["gene_variance_wasserstein"],
            "scCube_slideseq_pairwise_metrics_available": True,
            "source_lineage_complete": False,
            "interpretation": "the identity-preserving scCube Slide-seq rerun closes the missing named-support metrics; aggregate ranking remains unauthorized",
            "external_environment_limitation": rerun_provenance["external_method_environment_note"],
        },
    )

    output_hashes = {
        path.name: sha256(path)
        for path in (table_path, OUTPUT_ROOT / "moran_gene_panel.csv", comparison_path, support_path, decision_path)
    }
    ...
    write_json(
        ...
        {
            "schema_version": 1,
            "configuration_id": "study00-final-metrics-sccube-slideseq-identity-v1",
            "calculation_randomness": "none",
            "base_table": {"path": relative(base_path), "sha256": sha256(base_path)},
            "reference": {"artifact_id": rerun_provenance["input_artifact_id"], "path": relative(REFERENCE), "sha256": sha256(REFERENCE)},
            "external_candidate": {
                "configuration_id": rerun_provenance["configuration_id"],
                "method": "scCube",
                "version": rerun_provenance["method_version"],
                "public_seed": rerun_provenance["seed"],
                "path": relative(CANDIDATE),
                "sha256": sha256(CANDIDATE),
                "provenance_path": relative(rerun_provenance_path),
                "provenance_sha256": sha256(rerun_provenance_path),
                "solver_diagnostics": rerun_provenance["solver_diagnostics"],
                "external_environment_supported_by_feast": False,
            },
            "score_script_sha256": sha256(STUDY_ROOT / "score.py"),
            "builder_sha256": sha256(Path(__file__).resolve()),
            "outputs": output_hashes,
            "composite_score": "prohibited",
            "method_ranking": "prohibited_pending_author_review",
        },
    )

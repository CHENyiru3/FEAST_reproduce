...
METHOD_SPECS = (
    ...
    ("STAGATE_mclust", "mclust", "STAGATE + mclust"),
    ("Leiden_unsupervised", "predicted_cluster", "Label-free Leiden"),
)

# (simulation ID, display level).  Each factor is a complete ordered sequence
# with the neutral FEAST baseline in the middle, so spatial partitioning can be
# followed across the full perturbation range rather than at only two levels.
LEVEL_SPECS = {
    "mean": (
        ("mean_0_50", "FC\n0.50"),
        ("mean_0_67", "FC\n0.67"),
    ...
    "sparsity": (
        ("sparsity_neg_1", "Shift\n−1.00"),
        ("sparsity_neg_0_50", "Shift\n−0.50"),
        ("sparsity_neg_0_25", "Shift\n−0.25"),
        ("baseline", "FEAST baseline\nshift 0"),
        ("sparsity_pos_0_25", "Shift\n+0.25"),
        ("sparsity_pos_0_50", "Shift\n+0.50"),
        ("sparsity_pos_1", "Shift\n+1.00"),
    ),
}
...
# Matplotlib scatter area; a modest increase keeps individual spots clear in
# the seven-level small-multiple layout.
POINT_SIZE = 8.2
...
def read_panel(path: Path, label_key: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    try:
        ...
        return (
            coords[:, :2].astype(float),
            data.obs["ground_truth"].astype(str).to_numpy(),
            data.obs[label_key].astype(str).to_numpy(),
        )
    finally:
        data.file.close()
...
def local_cluster_colors(labels: np.ndarray) -> np.ndarray:
    ...
    label_to_color = {
        label: CAT_PALETTE[idx % len(CAT_PALETTE)]
...
def configure_matplotlib() -> None:
    plt.rcParams.update({
        ...
        "ps.fonttype": 42,
        "svg.fonttype": "none",
        "svg.hashsalt": "feast-study01-cluster-spatial-v2",
    })
...
def draw_one_figure(alter_type: str, output_dir: Path, dpi: int) -> list[Path]:
    ...
    truth_to_color = {
        label: CAT_PALETTE[idx % len(CAT_PALETTE)]
        for idx, label in enumerate(truth_order)
    }
    ...
    alter_title = {
        ...
        "variance": "Variance perturbation",
        "sparsity": "Zero-fraction perturbation",
    }
    fig.suptitle(
        f"{alter_title[alter_type]} across all registered levels — DLPFC slice {SLICE}",
        fontsize=14,
        fontweight="bold",
        x=0.54,
        y=0.985,
    ...
    fig.text(
        0.54,
        0.949,
        "Columns show increasing alteration level (neutral FEAST baseline centred); predicted colours encode local cluster labels",
        ha="center",
        va="center",
        fontsize=9.3,
        color="#666666",
    ...
    for suffix, kwargs in {
        "pdf": {"metadata": {"Creator": "FEAST Study 01 clustering spatial"}},
        "png": {"dpi": dpi, "metadata": {"Software": "FEAST Study 01 clustering spatial"}},
        "svg": {"metadata": {"Creator": "FEAST Study 01 clustering spatial"}},
    }.items():
...
def write_provenance(output_dir: Path, outputs: list[Path]) -> Path:
    input_paths = [result_path(method, sim_id) for sim_id, _label in LEVEL_SPECS["mean"]
                   for method, _key, _label in METHOD_SPECS]
    for alter_type in ("variance", "sparsity"):
        input_paths.extend(
            result_path(method, sim_id)
            for sim_id, _label in LEVEL_SPECS[alter_type]
            for method, _key, _label in METHOD_SPECS
        )
    payload = {
        "schema_version": 1,
        "figure_set": "study01_spatial_clustering",
        "display_slice": SLICE,
        "predicted_label_display": "Each predicted panel uses its own local categorical-label palette; no label remapping.",
        "source": {

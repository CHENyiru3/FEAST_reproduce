...
SIM_DIR = REPOSITORY_ROOT / '01_clustering' / 'outputs' / 'final_rerun_20260718' / 'simulations' / 'simulations' / '151676'
SLICE = '151676'
GENE = 'MBP'
COLUMN_SPECS = (('reference', 'Observed\nreference'), ('baseline', 'FEAST\nbaseline'), ('decrease', 'Decreased\ncondition'), ('increase', 'Increased\ncondition'))
ROW_SPECS = (('Mean', 'fold change 0.50 / 2.00', 'mean_0_50', 'mean_2'), ('Variance', 'fold change 0.50 / 2.00', 'variance_0_50', 'variance_2'), ('Zero fraction', 'logit shift −1.0 / +1.0', 'sparsity_neg_1', 'sparsity_pos_1'))
POINT_SIZE = 9.0
...
def draw_figure(output_dir: Path, output_stem: str, dpi: int) -> list[Path]:
    ...
    for row_idx, (row_label, detail, _decrease_id, _increase_id) in enumerate(ROW_SPECS):
        for col_idx, (column_key, _column_label) in enumerate(COLUMN_SPECS):
            ...
            scatter = ax.scatter(xy[:, 0], xy[:, 1], s=POINT_SIZE, c=expr_by_panel[row_label, column_key], cmap='viridis', vmin=0, vmax=vmax, marker='.', linewidths=0, rasterized=True)
            ax.set_aspect('equal')
            ax.invert_yaxis()
            ax.axis('off')
        axes[row_idx, 0].text(-0.24, 0.56, row_label, transform=axes[row_idx, 0].transAxes, ha='right', va='center', fontsize=12, fontweight='bold')
        axes[row_idx, 0].text(-0.24, 0.37, detail, transform=axes[row_idx, 0].transAxes, ha='right', va='center', fontsize=8.5, color='#666666')
    ...
    fig.suptitle(f'{GENE} expression — DLPFC slice {SLICE}', fontsize=14, fontweight='bold', x=0.47, y=0.985)
    cbar = fig.colorbar(scatter, ax=axes, fraction=0.023, pad=0.018)
    cbar.set_label(f'log1p({GENE} expression)\nshared 0–99.5th percentile scale', fontsize=10, labelpad=10)
    cbar.ax.tick_params(labelsize=9)
    fig.subplots_adjust(left=0.17, right=0.91, top=0.9, bottom=0.055, wspace=0.025, hspace=0.055)
    output_dir.mkdir(parents=True, exist_ok=True)
    stem = output_dir / output_stem
    paths: list[Path] = []
    for suffix, kwargs in {'pdf': {'metadata': {'Creator': 'FEAST Study 01 spatial expression'}}, 'png': {'dpi': dpi, 'metadata': {'Software': 'FEAST Study 01 spatial expression'}}, 'svg': {'metadata': {'Creator': 'FEAST Study 01 spatial expression'}}}.items():
...
def write_provenance(output_dir: Path, outputs: list[Path]) -> Path:
    ...
    payload = {'schema_version': 1, 'figure_set': 'study01_spatial_expression', 'display_slice': SLICE, 'marker_gene': GENE, 'normalization': 'Shared log1p scale, vmin=0, pooled 99.5th percentile vmax.', 'source': {'plot_script': str(Path(__file__).resolve().relative_to(REPOSITORY_ROOT)), 'inputs': {str(path.resolve().relative_to(REPOSITORY_ROOT)): None for path in input_paths}}, 'outputs': {path.name: None for path in sorted(outputs)}, 'scientific_disposition': {'figure_promotion_authorized': False, 'supported_interpretation': 'Descriptive spatial comparison of a registered representative slice.'}}

#!/usr/bin/env python3
"""Run the 40 Spateo/PASTE alignment jobs from a rotation manifest."""
...
def artifact_manifest_is_valid(
    output: Path,
    method: str,
    serialized_artifacts: object,
    runner_log_sha256: object,
) -> bool:
    try:
        artifacts = (
            serialized_artifacts
            if isinstance(serialized_artifacts, list)
            else json.loads(serialized_artifacts)
        )
    ...
    return bool(
        ...
        and sha256_file(runner_log) == runner_log_sha256
    )


def method_matrix_is_valid(
    output: Path,
    job: dict,
    diagnostics: dict,
    config: dict,
    ...
    try:
        ...
        expected_shape = (int(job["n_spots"]), int(job["n_spots"]))
        if (
            matrix.ndim != 2
    ...
    if job["method"] == "spateo":
        ...
        try:
            return bool(
                ...
                and np.isclose(
                    float(diagnostics.get("mapping_minimum")),
                    minimum,
                    rtol=1e-12,
                    atol=1e-12,
                )
                and np.isclose(
                    float(diagnostics.get("mapping_mass")),
                    total_mass,
                    rtol=1e-12,
                    atol=1e-12,
                )
                and float(diagnostics.get("mapping_negative_tolerance"))
                == negative_tolerance
            )
...
def load_jobs(rotation_dir: Path, reference: Path, methods: list[str]) -> list[dict]:
    manifest_path = rotation_dir / "rotation_manifest.csv"
    manifest = pd.read_csv(manifest_path)
    required = {
        "alteration", "angle_degrees", "output_path", "output_sha256", "n_spots",
    }
    if not required.issubset(manifest.columns) or len(manifest) != 20:
        raise RuntimeError("rotation manifest must contain the exact 20 prepared inputs")
    reference_hash = sha256_file(reference)
    jobs = []
    for row in manifest.to_dict("records"):
        moving = Path(row["output_path"])
        if not moving.is_file() or sha256_file(moving) != row["output_sha256"]:
            raise RuntimeError(f"rotation artifact hash mismatch: {moving}")
        for method in methods:
            jobs.append(
                {
                    "job_id": f"{row['alteration']}__{method}__angle_{row['angle_degrees']:g}",
                    "method": method,
...
def completed_job_is_valid(
    ...
    expected_configuration_id = (
        f"study02-{job['method']}-{job['alteration']}-{job['angle_degrees']:g}"
    )
    if not (
        record.get("moving_sha256") == job["moving_sha256"]
        and record.get("reference_sha256") == job["reference_sha256"]
        and record.get("configuration_id") == expected_configuration_id
        and record.get("config_sha256") == config_hash
        and record.get("method_python") == str(method_python.resolve())
        and record.get("output_dir") == str(output.resolve())
        and record.get("public_seed") == public_seed
        and record.get("feast_version") == job["feast_version"]
        and record.get("feast_commit") == job["feast_commit"]
    ):
        return False
    if not artifact_manifest_is_valid(
        output,
        job["method"],
        record.get("artifacts", "[]"),
        record.get("runner_log_sha256"),
    ):
        return False
    try:
    ...
    method_script = Path(__file__).parent / "methods" / (
        "run_spateo.py" if job["method"] == "spateo" else "run_paste.py"
    )
    if not (
        transform.get("status") == "ok"
        and transform.get("configuration_id") == expected_configuration_id
        and transform.get("public_seed") == public_seed
        and inputs.get("reference_sha256") == job["reference_sha256"]
        and inputs.get("moving_sha256") == job["moving_sha256"]
        and sources.get("wrapper_sha256") == sha256_file(method_script)
        and sources.get("config_sha256") == config_hash
        and method_matrix_is_valid(output, job, diagnostics, config)
    ):
        return False
    if job["method"] == "spateo":
        return bool(
    ...
    return bool(
        diagnostics.get("status") == "converged"
...
def main() -> int:
    ...
    if (
        len(input_manifest) != 1
        or reference_hash != str(input_manifest.iloc[0]["sha256"])
    ):
        raise RuntimeError("alignment reference does not match its declared input hash")
    ...
    if not (
        rotation_provenance.get("reference_sha256") == reference_hash
        and rotation_provenance.get("config_sha256") == config_hash
        and rotation_provenance.get("feast_commit") == config["required_feast_commit"]
        and int(rotation_provenance.get("public_seed", -1)) == public_seed
        and rotation_provenance.get("manifest_sha256")
        == sha256_file(rotation_manifest_path)
    ):
    ...
    all_jobs = load_jobs(args.rotation_dir, args.reference, ["spateo", "paste"])
    ...
    if args.resume and manifest_path.is_file():
        previous = {
            ...
            for record in pd.read_csv(manifest_path).to_dict("records")
        }
    results: list[dict] = []
    ...
    for record in previous.values():
        ...
        if job is None or not completed_job_is_valid(
            args.output_dir
            / job["alteration"]
            / job["method"]
            / f"angle_{job['angle_degrees']:g}",
            job,
            record,
            config_hash,
            config,
            interpreters[job["method"]],
            public_seed,
        ):
            raise RuntimeError(
                "subset resume cannot retain an unverified row from the other method; "
                "resume both methods with the current config and interpreters"
            )
    ...
    method_scripts = {
        "spateo": Path(__file__).parent / "methods" / "run_spateo.py",
        "paste": Path(__file__).parent / "methods" / "run_paste.py",
    }
    ...
    for index, job in enumerate(jobs, start=1):
        ...
        if args.resume and completed_job_is_valid(
            output,
            job,
            old,
            config_hash,
        ...
        command = [
            str(interpreters[method]), str(method_scripts[method]),
            "--reference", job["reference"], "--moving", job["moving"],
            "--output-dir", str(output), "--seed", str(public_seed),
            "--configuration-id", f"study02-{method}-{job['alteration']}-{job['angle_degrees']:g}",
        ...
        record = {
                **job,
                "configuration_id": command[command.index("--configuration-id") + 1],
                ...
                "method_python": str(interpreters[method].resolve()),
                "runner_log_sha256": sha256_file(log_path),
                "artifacts": json.dumps(artifacts, sort_keys=True),
            }
        if status == "ok" and not completed_job_is_valid(
            output,
            job,
            record,
            config_hash,
        ...
        write_manifest()
        print(
            f"[{index}/{len(jobs)}] {job['job_id']}: {record['status']}",
            flush=True,
        )
